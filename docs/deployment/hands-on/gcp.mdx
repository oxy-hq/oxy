...
<Step title="Create a VM Instance">
  ...
  Choose a name for your instance (e.g., `oxy-server`) - **Region & Zone**:
  ...
  plan to set up HTTPS later Click **Create** to provision your VM.
</Step>
...
<Step title="Add DNS Records">
  Add the following DNS records: - **A Record**: - Host/Name: `@` (represents
  ...
  (or as recommended by your registrar)
</Step>
...
<Step title="Alternative: Set Up Git and Clone Existing Project">
  <Note>
    ...
    set up Git and clone your repository instead of using `oxy init`.
  </Note>
  First, make sure Git is properly configured: ```bash git config --global
  user.name "Your Name" git config --global user.email "your.email@example.com"
  ``` Then clone your existing repository: ```bash git clone
  https://github.com/yourusername/your-oxy-project.git ~/oxy-workspace cd
  ~/oxy-workspace ``` This will give you your existing project configuration and
...
<AccordionGroup>
  <Accordion title="Cannot access Oxy through the domain or IP">
    ...
    rule target tags - Test local connection: `curl http://localhost:3000` - For
    domain issues: Run `dig yourdomain.com` to verify DNS is pointing to your
    VM's external IP - Check if your VM's external IP has changed (this can
    happen after stop/start)
  </Accordion>
...
<Accordion title="Data not persisting between restarts">
  ...
  <Accordion title="Connection refused or timeout errors">
    ...
    `sudo systemctl status oxy` - Test if the port is open: `sudo netstat -tlnp
    | grep :3000` - Verify the VM has the correct network tag applied
  </Accordion>
</AccordionGroup>
