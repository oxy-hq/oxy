...
<Tabs>
  ...
  <Tab title="Rancher Desktop">
    ...
    <Steps>
      ...
      <Step title="Windows (WSL): Ensure WSL 2 and Enable Integration">
        <Warning>
          **Windows users:** Rancher Desktop requires **WSL 2**. WSL 1 will not work — the Docker socket integration depends on WSL 2.
        </Warning>

        1. Check your WSL version from **PowerShell**:
           ```bash
           wsl -l -v
           ```
        2. If your distro shows VERSION 1, upgrade it:
           ```bash
           ...
           wsl --shutdown
           ```
        3. After confirming WSL 2, open Rancher Desktop and go to **Preferences → WSL → Integrations**
        4. Enable the toggle for your WSL distro (e.g., Ubuntu) and click **Apply**

        <Info>
          If the Integrations tab is empty (no distros listed), fully quit Rancher Desktop, run `wsl --shutdown` from PowerShell, then reopen Rancher Desktop. If it's still empty, uninstall and reinstall Rancher Desktop.
        </Info>
      </Step>
...
<Accordion title="Rancher Desktop on Windows (WSL): docker command not found or socket missing">
  This is almost always caused by WSL 1 or missing WSL integration. Follow these steps:

  1. **Verify you're on WSL 2** (from PowerShell):
  ...
  2. **Enable WSL Integration in Rancher Desktop:**
     - Go to **Preferences → WSL → Integrations**
     - Enable the toggle for your distro (e.g., Ubuntu)
     - Click **Apply**

  3. **If the Integrations tab is empty:**
     - Fully quit Rancher Desktop (right-click tray icon → Quit)
     - Run `wsl --shutdown` from PowerShell
     - Reopen Rancher Desktop and check again
     - If still empty, uninstall and reinstall Rancher Desktop

  4. **If Docker socket is not at the default path**, set `DOCKER_HOST`:
