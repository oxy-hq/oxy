...
2. **Docker Deployment** - Use our [Docker image](https://github.com/oxy-hq/oxy/pkgs/container/oxy) for a containerized deployment that simplifies setup and maintenance. See the [Docker deployment guide](/deployment/docker) for details.loyment Guide"
...
flowchart TD
    ...
    style DockerV fill:#f3e5f5
    style DockerE fill:#fff3e0
    style DockerR fill:#e8f5e8
    style E fill:#f3e5f5
    style F fill:#e3f2fd
    style G fill:#e3f2fd
    style H fill:#e3f2fd
...
<CardGroup cols={2}>
  ...
  <Card
    title="4. Set Up Workspace & Repository"
    icon="folder-tree"
...
For troubleshooting and advanced configuration, see our [support forum](https://github.com/oxy-hq/oxy/discussions) or contact the Oxy team.
...
echo "===== 3. Setting up Caddy reverse proxy ====="
...
# ===== STEP 4: Set Up Workspace & Repository =====
...
<Warning>
  This script is provided as a starting point and may need adjustments for your specific environment. Always review scripts before running them on your server.
</Warning>

To use the script:
1. Save it to a file (e.g., `deploy-oxy.sh`)
2. Make it executable: `chmod +x deploy-oxy.sh`
