...
flowchart TD
  ...
  AuthChoice{Authentication Required?}
  Auth[Set Up Authentication]
  AuthMethod{Auth Method?}
  Basic[Configure Basic Auth]
  OAuth2[Configure OAuth2 Proxy]
  ...
  Env --> Proxy
  Proxy --> AuthChoice
  AuthChoice -->|Yes| Auth --> AuthMethod
  AuthChoice -->|No| Done
  AuthMethod -->|Basic| Basic --> Done
  AuthMethod -->|OAuth2| OAuth2 --> Done
```
...
flowchart TD
    User[User/Browser]
    ReverseProxy["Caddy<br/>(Reverse Proxy, optional)"]
    OxyServer[Oxy Server]
    OAuth2[OAuth2 Provider]
    Workspace[Oxy Workspace]
    Data[Persistent Data]

    User -->|Web Request| ReverseProxy
    ReverseProxy -->|Proxy Request| OxyServer
    OxyServer -->|Authentication| OAuth2
    OAuth2 -->|Token/Claims| OxyServer
