...
# With enterprise features (ClickHouse telemetry, semantic engine)
oxy serve --cloud --enterprise
...
<Note>
  The `--enterprise` flag enables enterprise features including
  ClickHouse-backed telemetry and the semantic engine. Use this flag when your
  deployment includes a ClickHouse instance and OTel Collector.
</Note>
...
For Kubernetes, use the `oxy-app` Helm chart with the `--cloud --enterprise` command:

```yaml
# oxy-cloud values.yaml
app:
  image: ghcr.io/oxy-hq/oxy
  imageTag: "0.5.20"
  imagePullPolicy: IfNotPresent
  command: ["oxy", "serve", "--cloud", "--enterprise"]
...
# Cloud mode fetches project files from GitHub on demand — no gitSync needed
gitSync:
  enabled: false

env:
  OXY_STATE_DIR: "/workspace/oxy_data"
  # OXY_DATABASE_URL injected via external secret
  OXY_API_URL: https://app.your-domain.com/api
  MAGIC_LINK_FROM_EMAIL: "noreply@your-domain.com"
  MAGIC_LINK_AWS_REGION: "us-west-2"

# Inject database URL and API keys from your secrets manager
externalSecrets:
  envSecretNames: ["oxy-cloud-env-secret"]
...
```bash
helm install oxy-cloud oci://ghcr.io/oxy-hq/helm-charts/oxy-app \
  --namespace oxy-app \
  --create-namespace \
  --version 0.4.2 \
  --values _base.yaml \
  --values oxy-cloud-values.yaml
```
