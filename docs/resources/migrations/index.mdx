...
description: "Migrate your Oxygen installation from SQLite to PostgreSQL using simple CLI commands"
---
...
Starting with version 0.4.0, Oxygen uses PostgreSQL as its primary database. This guide will help you migrate your existing SQLite data to PostgreSQL using Oxygen's built-in migration commands.

# Why PostgreSQL?

Oxygen has migrated to PostgreSQL to provide:
...
Oxygen now offers three convenient CLI commands for database management:

- **`oxy start`**: Automatically starts PostgreSQL in Docker and launches the Oxygen server
...
2. **Oxygen 0.4.0 or later** - Check with `oxy --version`
...
### Step 7: Use Oxygen with PostgreSQL
...
The server will connect to the PostgreSQL database and you can use Oxygen normally.
...
Start PostgreSQL in Docker and launch the Oxygen server.
...
5. Starts the Oxygen web server
...
A: No, Oxygen 0.4.0+ only supports PostgreSQL. Your old SQLite file remains intact if you need to downgrade to an earlier version.
...
Migrating to PostgreSQL with Oxygen's CLI commands is straightforward:
