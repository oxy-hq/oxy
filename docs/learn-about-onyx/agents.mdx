...
Warehouse information can be accessed within `system_instructions` by using the `warehouses` namespace, then referencing by `name`, as follows:

```
**Dataset: {{ warehouses.primary_warehouse.name }}**
**Dialect: {{ warehouses.primary_warehouse.dialect }}**
**Tables:**
{% for table in warehouses.primary_warehouse.tables %}
{{ table }}
{% endfor %}
```
...
system_instructions: |
  ...
  - Ensure the SQL query adheres to the **specific dialect of the database warehouse** being used.
