...
- **`OXY_OBSERVABILITY_BACKEND`** — turns span **capture** on. Set it to `duckdb`, `postgres`, or `clickhouse` and Oxygen starts writing traces to that store. No capture happens if it's unset.
...
<Note>
**DuckDB** is fastest for small-to-medium volumes and needs zero extra infrastructure. The trade-off is it only supports a single writer, so it can't be shared across multiple Oxygen pods.

**Postgres** is the right pick for horizontally-scaled deployments. It reuses the connection pool Oxygen already uses for its own state, so there's no extra database to provision.
