...
<Note>
  **For operators.** This guide is for the person who installs and configures the Oxygen server. End users who want to import a repository just click **Import from GitHub** in the Workspaces UI — they don't need to read this.
</Note>

## Overview

In multi-workspace mode, Oxygen uses a **GitHub App** (not a personal OAuth app) to:
...
2. Fill in the app details:
   ...
   | **Homepage URL** | Your Oxygen instance URL (e.g. `https://app.example.com`) |
   | **Callback URL** | `https://your-domain.com/github/callback` |
   | **Webhook** | Uncheck "Active" — Oxygen does not use GitHub webhooks |
...
Set the following on your Oxygen server:
...
Oxygen derives the OAuth callback URL automatically from the `Origin` header of the browser request — no hardcoding is needed. However, ensure:
...
2. **Your reverse proxy forwards the `Origin` header** to the Oxygen backend. Most proxies (nginx, Caddy, ALB) do this by default.
...
When a user clicks **Import from GitHub** in the Workspaces UI, Oxygen walks them through connecting their GitHub account and selecting a repository.

### Step 1 — Connect a GitHub account

Oxygen opens a GitHub OAuth popup. The user authorizes the app, and GitHub redirects back with a short-lived code that Oxygen exchanges for an access token. This token is used to discover which GitHub App installations are visible to that user.

There are three possible outcomes:

| Outcome | What the user sees |
|---------|-------------------|
| **One installation found** | Oxygen auto-connects it — the user skips straight to repository selection |
...
Once an installation (GitHub account or organization) is connected, the user sees the repositories that the GitHub App has access to. They pick one and Oxygen clones it as a new workspace.
...
When multiple installations are shown, Oxygen issues a short-lived **selection token** — an HMAC-signed value encoding the eligible installation IDs, bound to the user's ID and valid for 10 minutes. When the user picks one:
...
only org owners and admins are included; regular members get an empty list, which Oxygen reports as `not_installed`.
...
The connect popup opens in a new browser window. If the user is already signed into GitHub as a different account in that window, they may need to sign out of GitHub first before authorizing. After a successful authorization, Oxygen always uses the most recently connected account.
