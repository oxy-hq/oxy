...
Once you have your agents, config, and data configured, you can ask this agent questions by running `oxy run path/to/agent.agentic.yml "<prompt>"`, as shown below:

```bash
oxy run path/to/agent.agentic.yml "How many users do we have?"
```
...
```bash
oxy test agent-name.agentic.yml
