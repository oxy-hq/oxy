...
<ParamField path="--readonly" type="boolean" default="false">
  Prevent all file modification operations via the API. The server starts normally but
  returns `403 Forbidden` for any request that would write to project files or change
