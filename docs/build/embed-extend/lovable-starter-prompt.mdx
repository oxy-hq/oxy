...
description: "A ready-to-use prompt for scaffolding an Oxygen SDK dashboard in Lovable"
---
...
Build a dashboard that connects to a local Oxygen backend to display monthly metrics in real-time.

**Goal**: Demonstrate the simplest possible Oxygen SDK integration pattern - querying time-series data (month + value).
...
export async function fetchMonthlyMetrics(
  ...
  try {
    console.log("[Oxygen] Loading app data...");
    await sdk.loadAppData(appPath);

    console.log("[Oxygen] Querying table...");
    const result = await sdk.query("SELECT * FROM monthly_values");

    console.log("[Oxygen] Columns:", result.columns);
    console.log("[Oxygen] Number of rows:", result.rows?.length);
    ...
    result.rows.forEach((row: any[], i: number) => {
      ...
      console.log(`[Oxygen] Row ${i}: ${cellInfo.join(" | ")}`);
    });
    ...
    console.error("[Oxygen] Error:", error);
    throw error;
  }
}
```
      ...
      <div className="w-full max-w-lg space-y-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Unable to connect to Oxygen backend</p>
          </div>
        </div>
