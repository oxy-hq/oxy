...
Add tests inline in your `.agent.yml` file (the same place the agent
lives):
...
Run the test from the **Tests** dashboard in the Oxygen IDE — open the
agent file, click **Run tests**, and Oxygen executes each prompt the
configured number of times and surfaces a final accuracy score plus
any consistency errors the judge LLM finds.

To automate the same run from CI, call the
[Tests REST API](/guide/reference/rest-api/tests).
...
For complete documentation on testing features, see the [Testing Guide](/guide/build/test-evaluate/testing).
