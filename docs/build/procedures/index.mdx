...
A **Procedure** is a series of tasks (`tasks`) executed sequentially.
Each `task` is either a deterministic command (such as `execute_sql`,
which runs a named SQL query) or an agent given a prompt. Tasks compose
by passing results from one to the input of another — the output of
each task is accessible with Jinja as `{{ name_of_task }}`.

You build, run, and replay Procedures from the Oxygen IDE. Each
Procedure is stored as a `.procedure.yml` file (older `.workflow.yml`
and `.automation.yml` extensions are also recognized).
...
See the [Semantic Layer](/guide/build/semantic-layer) for more information.
...
of the system. You can think of Oxygen's `workflow` paradigm as a domain-specific
