...
5. **Reader + handler wiring** — add `resolve_foo` (and `list_foos` if applicable) to `crates/app/src/server/api/compiled_reader.rs`. They follow the `open_compiled_revision` → `find_by_id` shape. Then wire the runtime handler:
   - If the handler accepts a string / struct: read the compiled row, deserialise from the JSONB definition, fall through to FS on miss. Gate on `feature_flags::is_enabled("compile_runtime_use_postgres")`.
...
The kill switch `compile_boundary_disabled` and the master flag `compile_runtime_use_postgres` already gate every reader through `open_compiled_revision` — you do not need to re-implement them per-surface.
...
- `internal-docs/2026-06-10-compile-boundary-simplification.md` — the design this PR implements.
- `internal-docs/2026-06-10-compile-boundary-architecture.md` — the architecture diagram.
