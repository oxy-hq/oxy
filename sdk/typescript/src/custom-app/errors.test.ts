...
import { interpretCustomAppError } from "./errors";

describe("interpretCustomAppError", () => {
  describe("manifest", () => {
    it("404 → manifest not found", () => {
      const r = interpretCustomAppError(new Error("Failed to load oxy-app.json: HTTP 404"));
      expect(r.title).toBe("Manifest not found");
    });

    it("network failure → manifest could not be loaded", () => {
      const r = interpretCustomAppError(new Error("Failed to load oxy-app.json: NetworkError"));
      expect(r.title).toBe("Manifest could not be loaded");
    });

    it("v1 manifest → schema mismatch", () => {
      const r = interpretCustomAppError(new Error("oxy-app.json: schemaVersion must be 2 (got 1)"));
  ...
  describe("/query 401/403/404", () => {
    it("401 → session expired", () => {
      const r = interpretCustomAppError(new Error("401: unauthorized"));
      expect(r.title).toBe("Session expired");
    });

    it("403 origin not allowed → CSRF branch", () => {
      const r = interpretCustomAppError(new Error("403: origin not allowed"));
      expect(r.title).toBe("Request origin not allowed");
    });

    it("403 not a member → access denied branch", () => {
      const r = interpretCustomAppError(new Error("403: forbidden: not a member of this org"));
      expect(r.title).toBe("Access denied");
    });

    it("403 SELECT-only gate → read-only branch", () => {
      const r = interpretCustomAppError(new Error("403: only SELECT/WITH allowed; reject DROP"));
      expect(r.title).toBe("Query rejected — read-only endpoint");
    });

    it("404 project → project not found", () => {
      const r = interpretCustomAppError(new Error("404: project not found"));
  ...
  describe("/query 400/500/502", () => {
    it("400 empty SQL → empty SQL branch", () => {
      const r = interpretCustomAppError(new Error("400: `sql` must be non-empty"));
      expect(r.title).toBe("Empty SQL");
    });

    it("400 query failed → query failed", () => {
      const r = interpretCustomAppError(
    ...
    it("502 → warehouse unreachable", () => {
      const r = interpretCustomAppError(new Error("502: warehouse connection failed"));
  ...
  describe("catch-all", () => {
    it("unknown error → generic", () => {
      const r = interpretCustomAppError(new Error("something weird"));
      expect(r.title).toBe("Unexpected error loading the dashboard");
    });

    it("non-Error → coerces to string", () => {
      const r = interpretCustomAppError("plain string");
