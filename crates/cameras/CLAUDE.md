...
- **Postgres entities + migrator** for the `Fleet` aggregate
  (Site → EdgeBox → EdgeBoxToken, Site → Camera).
...
| Concern | Location |
|---|---|
| SeaORM entities | `entities/{sites,edge_boxes,cameras,edge_box_tokens}.rs` |
...
| `edge_box_tokens` | Postgres | Auth — must be in app DB |
