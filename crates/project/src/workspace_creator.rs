use crate::config_builder::ConfigBuilder as ProjectConfigBuilder;
use crate::database_operations::DatabaseOperations;
use crate::git_service::GitService;
use crate::models::{AgentConfig, CreateWorkspaceRequest};
...
use oxy::service::sync::sync_databases;
...
impl WorkspaceCreator {
    ...
    async fn create_project_agent(
        ...
        use oxy::config::model::{AgentToolsConfig, AgentType, ToolType};
        ...
        let agent_type = AgentType::Default(oxy::config::model::DefaultAgent {
            system_instructions: agent.system_instructions.clone(),
            tools_config: Default::default(),
        });

        let mut agent_config = oxy::config::model::AgentConfig {
        ...
        if let Some(tools) = &agent.tools {
            ...
            for tool in tools {
                let tool_type = match tool.r#type.as_str() {
                    "execute_sql" => {
                        let execute_sql_config = oxy::config::model::ExecuteSQLTool {
                    ...
                    "visualize" => {
                        let visualize_config = oxy::config::model::VisualizeTool {
                            name: tool.name.clone(),
                            description: tool.description.clone(),
                        };
    ...
    async fn create_project_and_repo_records(
        workspace_id: Uuid,
        repo: &oxy::github::GitHubRepository,
