...
//!
//! To run locally: `OXY_DATABASE_URL=postgres://... cargo nextest run -p oxy-app --test slack -E 'test(slack_user_links)'`
...
async fn seed_installation() -> (Uuid, Uuid, Uuid) {
    ...
    users::ActiveModel {
        ...
        email: ActiveValue::Set(Some(format!("test-{}@example.com", user_id))),
