        ...
        Some(SubCommand::Build(build_args)) => {
            ...
            if semantic_dir.exists() {
                ...
                if changes.requires_full_rebuild {
                    ...
                    // Clear directory - preserve only manifest if not forced
                    if build_args.force {
                        // Force rebuild: clear everything including .cubestore
                        clear_directory_contents_selective(&target_dir, &[".build_manifest.json"])?;
                    } else {
                        // Natural full rebuild (globals/config changed): preserve .cubestore
                        clear_directory_contents_selective(
                            &target_dir,
                            &[".cubestore", ".build_manifest.json"],
                        )?;
                    }
                ...
                } else if !changes.views_to_rebuild.is_empty()
                    || !changes.topics_to_rebuild.is_empty()
                    || !changes.files_to_delete.is_empty()
                {
                    // Incremental rebuild of semantic layer
                    oxy_semantic::cube::translator::process_semantic_layer_incremental(
                        semantic_dir,
                        target_dir,
                        databases,
                        config_manager.get_globals_registry(),
                    ...
                    .await
                    .map_err(|e| {
                        OxyError::RuntimeError(format!("Incremental build failed: {}", e))
                    })?;
                    ...
                    println!("✅ Semantic layer is up to date");
                }
            }
        }
