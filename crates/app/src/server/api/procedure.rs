//! Compile-boundary-backed procedure (a.k.a. workflow) listing.
//!
//! The customer-nav sidebar's "Procedures" list must render on a stateless serve
...
/// One procedure file, matching the `agentic-http` `WorkflowFile` shape the FE
...
fn to_file(relative_path: String) -> ProcedureFile {
...
/// `GET /{workspace_id}/procedures` — FleetOk. Serves the procedure list from the
...
) -> Result<extract::Json<Vec<ProcedureFile>>, StatusCode> {
    match compiled_reader::list_procedure_artifacts(
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::warn!(
                ...
                error = ?e,
                "list_procedures compile-boundary error; falling through to FS"
            );
        }
    }
...
/// One procedure's raw YAML — matches the `agentic-http` `WorkflowFileContent`
...
/// `GET /{workspace_id}/procedures/{path_b64}` — FleetOk. Serves one procedure's
/// YAML from the compile boundary so clicking a procedure renders its diagram on
...
pub async fn get_procedure(
    ...
    Query(query): Query<ListProceduresQuery>,
) -> Result<extract::Json<ProcedureContent>, StatusCode> {
    let file_path = decode_path_b64(&path_b64).ok_or(StatusCode::BAD_REQUEST)?;

    match compiled_reader::resolve_procedure(
        workspace_manager.workspace_id,
        query.branch.as_deref(),
        &file_path,
    )
    .await
    {
        Ok(Some(artifact)) => {
            ...
            // — the FE parses this for the procedure diagram. It is NOT a
            ...
            let content = serde_yaml::to_string(&artifact.definition).map_err(|e| {
                tracing::error!(file_path, "serialise compiled procedure: {e}");
                StatusCode::INTERNAL_SERVER_ERROR
            })?;
            return Ok(extract::Json(ProcedureContent {
                path: file_path,
                content,
            }));
        }
        ...
        Err(e) => {
            tracing::warn!(
                workspace_id = %workspace_manager.workspace_id,
                error = ?e,
                "get_procedure compile-boundary error; falling through to FS"
            );
        }
    }
    ...
    let content = tokio::fs::read_to_string(&full_path)
        .await
        .map_err(|_| StatusCode::NOT_FOUND)?;
    Ok(extract::Json(ProcedureContent {
        path: file_path,
        content,
    }))
}
