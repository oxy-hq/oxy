//! Reverse-proxy backend for remote-hosted customer apps.
//!
//! When a customer-app row's `source_type = "v0"` (i.e. the bundle is
...
pub async fn proxy(
    ...
    let Some(url) = build_upstream_url(upstream_base, rest, uri.query()) else {
        tracing::warn!(
            "customer-app proxy: invalid upstream URL (base={upstream_base:?} rest={rest:?})"
        );
    ...
    // Collect the request body into bytes — fine for v1: customer apps are
    ...
    let body_bytes = match axum::body::to_bytes(body, REQUEST_BODY_LIMIT).await {
        Ok(b) => b,
        Err(e) => {
            tracing::warn!("customer-app proxy: failed to buffer request body: {e}");
    ...
    let upstream = match upstream {
        ...
        Err(err) => {
            tracing::warn!(
                "customer-app proxy: upstream call failed (app={} url={url} err={err})",
                identity.app.id,
            );
            return (
                StatusCode::BAD_GATEWAY,
                "upstream customer-app deploy is unreachable",
            )
                .into_response();
        }
    };
...
/// admin) could point a customer app at `http://169.254.169.254/...` (cloud
...
fn build_outbound_headers(
    ...
    if public_host.is_empty() {
        ...
        tracing::debug!(
            "customer-app proxy: no x-forwarded-host / host on inbound request (app={})",
            identity.app.id
        );
    }
