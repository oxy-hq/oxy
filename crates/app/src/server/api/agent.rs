...
pub async fn get_agents(
    ...
    // branch and the kill-switch flag is off, serve the listing from
    // `agent_definitions`. `llm.ref` is pulled from the JSONB to keep
    // the same response shape without re-parsing every `.agentic.yml`.
    // `compiled_reader` handles the branch carve-out + kill switch
    // internally; on any error we fall through to FS (best-effort).
