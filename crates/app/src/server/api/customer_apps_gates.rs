//! Shared security gates for customer-app HTTP endpoints.
//!
...
/// minimum set of state every customer-app endpoint needs.
...
pub async fn check_customer_app_gates(
    headers: &HeaderMap,
    project_id: Uuid,
) -> Result<CustomerAppContext, Response> {
    // ── 1. Authenticate ───────────────────────────────────────────────
    ...
    if !is_allowed_origin(headers) {
        warn!("customer-app gate: request rejected — origin not in allowlist");
...
/// Parse a customer-app endpoint's JSON body, enforcing the
