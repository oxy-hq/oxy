...
//! login / legacy-uuid redirects for customer-app serving.
...
pub(super) fn redirect_to_login(headers: &HeaderMap, uri: &Uri) -> Redirect {
    ...
    let host = headers
        ...
        .unwrap_or("");
    let login_base = if crate::server::api::customer_apps_host_dispatch::parse_subdomain(host)
        .is_some()
    {
        crate::server::api::customer_apps_host_dispatch::admin_base_url().unwrap_or(request_base)
    } else {
        request_base
    };
    let target = format!(
