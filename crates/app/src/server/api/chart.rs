...
/// - When the ide is UNREACHABLE, a serve replica runs this handler locally (via
///   `role_manifest::degrades_when_ide_unreachable`), misses on disk, and serves
///   the S3 mirror — so a chart you've already viewed keeps loading while Oxygen
///   Factory restarts, instead of a hard 502.
///
...
pub async fn get_chart(
    ...
    // Fast path: the file is on THIS node's disk (the ide that ran the visualize
        ...
        && let Ok(content) = std::fs::read_to_string(charts_dir.join(&file_path))
    {
        ...
        // and `mirror` is a no-op when no bucket is configured.
        if let Some(key) = safe_key {
            let bytes = content.clone().into_bytes();
            tokio::spawn(async move {
                runtime_artifact::mirror(&key, bytes, "application/json").await;
            });
        }
