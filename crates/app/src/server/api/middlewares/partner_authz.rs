...
async fn is_oxy_staff(db: &DatabaseConnection, email: &str) -> bool {
    crate::server::authz::globals::platform_standing(db, email)
        .await
        .is_staff()
}
...
pub async fn partner_grants_app_access(
    ...
    crate::server::authz::partner_allows(&scope, Some(org_id), PartnerCapability::DevelopApps)
}
...
pub async fn assumed_partners(
    ...
    let assumed = crate::server::api::admin::assume::live_assumed_org_ids(db, user_id).await;
    if assumed.is_empty() {
        return Vec::new();
    }
