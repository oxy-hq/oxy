...
//! `@oxy-hq/sdk/shell` chrome inside customer-app bundles.
//!
...
//! - a **customer-app subdomain** (`<org>--<slug>.customer-apps.<zone>`)
...
use crate::server::api::customer_apps_host_dispatch::{admin_base_url, parse_subdomain};
...
/// on a customer-app subdomain, where no SPA exists and links must point
...
mod tests {
    ...
    #[test]
    fn absolute_base_on_customer_app_subdomain() {
        // Env-dependent half (`admin_base_url`) is covered in
        // customer_apps_host_dispatch tests; here we only assert the
