...
//!    events. Reuses the existing customer-app gate chain (cookie OR
//!    bearer auth, org-membership / app-admin access check), then
//!    validates the event shape and inserts via
//!    [`customer_apps_tracking::record_event`]. Rate limited per
...
use super::customer_apps_gates::check_customer_app_gates;
use super::customer_apps_tracking::{self, EventError};
...
pub async fn post_event(
    ...
    let session_id = req
        .session_id
        .or_else(|| customer_apps_tracking::session_id_from_headers(&headers))
        .unwrap_or_else(Uuid::new_v4);

    let result = customer_apps_tracking::record_event(
        &db,
        app.id,
        ctx.user.id,
