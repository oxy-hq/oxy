...
fn is_local_admin_from_config(email: &str) -> bool {
    ...
    match serde_yaml::from_str::<AdminsOnly>(&yaml) {
        Ok(ac) => ac.admins.is_empty() || ac.admins.contains(&email.to_string()),
        Err(_) => true, // Parse error → permissive
    }
}
