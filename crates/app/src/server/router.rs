...
/// 2. If `config.admins` is non-empty, the email must be listed there.
/// 3. If `config.admins` is empty/absent, all users are admin (permissive default).
fn is_local_admin(email: &str, request: &Request<Body>) -> bool {
    // Built-in local guest is always admin
    if email == "<local-user@example.com>" {
        return true;
    }

    // Try to get admins list from project config (set by project_middleware)
    if let Some(project_manager) = request.extensions().get::<ProjectManager>() {
        let admins = &project_manager.config_manager.get_config().admins;
        return admins.is_empty() || admins.contains(&email.to_string());
    }

    // If project manager not available, default to permissive (allow)
    true
}
