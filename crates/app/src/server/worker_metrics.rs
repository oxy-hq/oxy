...
//! within a target band. See `internal-docs/2026-06-06-compile-boundary-design.md`
...
pub async fn metrics(State(state): State<MetricsState>) -> Response {
    ...
    let db_error = queue_rows.is_err();
...
async fn read_queue_depth(
    db: &DatabaseConnection,
) -> Result<Vec<QueueDepthRow>, sea_orm::DbErr> {
