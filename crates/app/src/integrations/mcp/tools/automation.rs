...
fn automation_display_name(path: &std::path::Path) -> String {
    let stem = path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("workflow");
...
pub async fn get_all_automation_tools(
    ...
    for path in paths {
        let name = automation_display_name(&path);
...
pub async fn resolve_automation_tool(
    ...
    let relative_path = automation_path
        ...
        .to_path_buf();
    let name = automation_display_name(&relative_path);
    let tool_name = get_automation_tool_name(&name);
    let tool = Tool::new(tool_name.clone(), "", Arc::new(variables_only_schema()));
    Ok((
        tool_name,
...
pub async fn run_automation_tool(
    ...
    let automation_path = paths
        .into_iter()
        .find(|p| automation_display_name(p) == automation_name)
        .ok_or_else(|| {
