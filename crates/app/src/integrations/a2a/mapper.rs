...
use oxy::execute::types::{Output, OutputContainer, Table};
...
pub fn oxy_error_to_task_status(error: &oxy_shared::errors::OxyError) -> a2a::types::TaskStatus {
...
pub fn create_task_failed_event(
    task_id: &str,
    error: &oxy_shared::errors::OxyError,
) -> a2a::streaming::SseEvent {
    let event_data = serde_json::json!({
        "task_id": task_id,
