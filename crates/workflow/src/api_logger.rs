...
use crate::streaming_workflow_persister::StreamingWorkflowPersister;
use oxy::execute::types::Table;

use crate::logger_types::{LogItem, LogType, WorkflowLogger};
...
impl WorkflowAPILogger {
    ...
    fn log(&self, log_item: LogItem) {
        ...
        if let Some(streaming_handler) = &self.streaming_persister {
            ...
            // Convert workflow crate's LogItem to core's LogItem via serde
            let log_item_json = serde_json::to_value(&log_item).unwrap();
            let core_log_item: crate::logger_types::LogItem =
