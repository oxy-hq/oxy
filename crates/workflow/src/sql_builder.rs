    ...
    async fn map(
        ...
        if let Some(vars) = &input.variables {
            for (key, value) in vars {
                let rendered_value = execution_context.renderer.render_str(value)?;
        ...
        let sql = match &input.sql {
            SQL::Query { sql_query } => {
                let query = execution_context.renderer.render_str(sql_query)?;
            ...
            SQL::File { sql_file } => {
                let rendered_sql_file = execution_context.renderer.render_str(sql_file)?;
