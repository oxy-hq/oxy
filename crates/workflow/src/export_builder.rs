...
use oxy::{
    config::model::{
        AgentTask, ExecuteSQLTask, ExportFormat, FormatterTask, OmniQueryTask, SemanticQueryTask,
        TaskExport, TaskType,
    },
    execute::{
        ExecutionContext,
...
use crate::task_builder::TaskInput;
...
impl Exporter<TaskInput, OutputContainer> for TaskExporter {
    async fn should_export(
        ...
        let export_info = match &input.task.task_type {
            ...
            TaskType::OmniQuery(OmniQueryTask { export, .. }) => export,
    ...
    async fn export(
        ...
        let (export_info, prompt) = match input.task.task_type {
            ...
            TaskType::OmniQuery(OmniQueryTask { export, .. }) => (export, String::new()),
        ...
        let path_resolver: Result<String, OxyError> = {
            let rendered_path = execution_context.renderer.render_str(&export_info.path)?;
