...
impl ChangeDetector {
    ...
    pub fn detect_changes(
        ...
        let mut topics_to_rebuild = Vec::new();
        ...
        for (file_path, current_hash) in &current_files {
            ...
            if prev_hash.is_none() || prev_hash.unwrap() != current_hash {
                ...
                match Self::classify_file(file_path) {
                    Some(FileType::View(name)) => {
                        if !views_to_rebuild.contains(&name) {
                            views_to_rebuild.push(name);
                        }
                    }
                    Some(FileType::Topic(name)) => {
                        if !topics_to_rebuild.contains(&name) {
                            topics_to_rebuild.push(name);
                        }
                    }
                    None => {
                        tracing::warn!("Unknown file type: {}", file_path);
                    }
                }
            }
        }
        ...
        for (old_file, _) in &manifest.file_hashes {
            if !current_files.contains_key(old_file) {
                // File was deleted
                if let Some(outputs) = manifest.output_mapping.get(old_file) {
                    for output in outputs {
                        files_to_delete.push(PathBuf::from(output));
                    }
                }

                // Also mark the view/topic for rebuild (to remove from dependency graph)
                match Self::classify_file(old_file) {
                    Some(FileType::View(_name)) => {
                        // Don't add to rebuild list - it's deleted
                    }
                    Some(FileType::Topic(_name)) => {
                        // Don't add to rebuild list - it's deleted
                    }
                    None => {}
                }
        ...
        let all_views = Self::expand_dependencies(&views_to_rebuild, &manifest.dependency_graph);
    ...
    /// Expand dependencies to include transitive dependents
    ///
    /// Uses BFS to find all views that transitively depend on the changed views
    fn expand_dependencies(
        changed_views: &[String],
        dependency_graph: &BTreeMap<String, Vec<String>>,
    ) -> Vec<String> {
        let mut to_rebuild: HashSet<String> = changed_views.iter().cloned().collect();
        let mut visited: HashSet<String> = HashSet::new();
        let mut queue: Vec<String> = changed_views.to_vec();
        ...
        while let Some(view) = queue.pop() {
            ...
            // In the dependency graph: key depends on values
            // So we need to find all keys where this view is in the values
            for (dependent, dependencies) in dependency_graph {
                if dependencies.contains(&view) && !visited.contains(dependent) {
                    to_rebuild.insert(dependent.clone());
...
pub fn hash_database_config(
    ...
    let json_str = serde_json::to_string(databases).unwrap_or_default();
...
mod tests {
    ...
    fn test_expand_dependencies() {
        ...
        let changed = vec!["customers".to_string()];
        let expanded = ChangeDetector::expand_dependencies(&changed, &dep_graph);

        assert!(expanded.contains(&"customers".to_string()));
        assert!(expanded.contains(&"orders".to_string()));
        assert!(expanded.contains(&"shipments".to_string()));
    }
    ...
    fn test_expand_dependencies_no_deps() {
        ...
        let dep_graph = BTreeMap::new();
        let changed = vec!["orders".to_string()];
        let expanded = ChangeDetector::expand_dependencies(&changed, &dep_graph);
    ...
    #[test]
    fn test_expand_dependencies_circular() {
        use std::collections::BTreeMap;

        let mut dep_graph = BTreeMap::new();
        // Circular dependency: A depends on B, B depends on A
        dep_graph.insert("view_a".to_string(), vec!["view_b".to_string()]);
        dep_graph.insert("view_b".to_string(), vec!["view_a".to_string()]);

        let changed = vec!["view_a".to_string()];
        let expanded = ChangeDetector::expand_dependencies(&changed, &dep_graph);

        // Should handle circular deps without infinite loop
        assert!(expanded.contains(&"view_a".to_string()));
        assert!(expanded.contains(&"view_b".to_string()));
        assert_eq!(expanded.len(), 2);
    }

    #[test]
    fn test_expand_dependencies_diamond() {
        use std::collections::BTreeMap;

        let mut dep_graph = BTreeMap::new();
        // Diamond dependency: D depends on B and C, both depend on A
        dep_graph.insert("view_b".to_string(), vec!["view_a".to_string()]);
        dep_graph.insert("view_c".to_string(), vec!["view_a".to_string()]);
        dep_graph.insert(
            "view_d".to_string(),
            vec!["view_b".to_string(), "view_c".to_string()],
        );

        let changed = vec!["view_a".to_string()];
        let expanded = ChangeDetector::expand_dependencies(&changed, &dep_graph);

        // Should rebuild all dependent views
        assert!(expanded.contains(&"view_a".to_string()));
        assert!(expanded.contains(&"view_b".to_string()));
        assert!(expanded.contains(&"view_c".to_string()));
        assert!(expanded.contains(&"view_d".to_string()));
