...
/// Process semantic layer incrementally, only rebuilding changed views
///
/// This function performs selective parsing and translation based on the change detection
/// result. It's more efficient than full rebuild for small changes.
///
/// # Arguments
/// * `semantic_dir` - Path to semantic layer source directory
/// * `target_dir` - Path to CubeJS output directory
/// * `databases` - Database configuration map
/// * `globals_registry` - Global variables registry
/// * `changes` - Result of change detection analysis
///
/// # Returns
/// * `Ok(())` on success
/// * `Err(SemanticLayerError)` on failure
pub async fn process_semantic_layer_incremental(
    semantic_dir: PathBuf,
    target_dir: PathBuf,
    databases: HashMap<String, DatabaseDetails>,
    globals_registry: GlobalRegistry,
    changes: crate::ChangeDetectionResult,
) -> Result<(), SemanticLayerError> {
    use crate::BuildManifest;
    use crate::change_detector::hash_database_config;
    use crate::cube::entity_graph::EntityGraph;

    println!(
        "🔧 Incremental build: {} views, {} topics",
        changes.views_to_rebuild.len(),
        changes.topics_to_rebuild.len()
    );

    // Delete orphaned output files first
    for file in &changes.files_to_delete {
        if file.exists() {
            std::fs::remove_file(file).ok(); // Ignore errors
            println!("   🗑️  Deleted: {}", file.display());
        }
    }

    // Parse ALL views to build complete EntityGraph (needed for joins)
    // We need the full graph even for incremental builds to compute correct joins
    let full_parse =
        crate::parse_semantic_layer_from_dir(semantic_dir.clone(), globals_registry.clone())
            .map_err(|e| {
                SemanticLayerError::ParsingError(format!("Failed to parse semantic layer: {}", e))
            })?;

    let entity_graph = EntityGraph::from_semantic_layer(&full_parse.semantic_layer)?;

    // Filter to only views that need rebuilding
    let views_to_process: Vec<_> = full_parse
        .semantic_layer
        .views
        .into_iter()
        .filter(|v| changes.views_to_rebuild.contains(&v.name))
        .collect();

    // Filter to only topics that need rebuilding
    let topics_to_process: Vec<_> = full_parse
        .semantic_layer
        .topics
        .map(|topics| {
            topics
                .into_iter()
                .filter(|t| changes.topics_to_rebuild.contains(&t.name))
                .collect()
        })
        .unwrap_or_default();
    ...
    for view in &views_to_process {
        println!("   - {}.view.yml", view.name);
    }
    for topic in &topics_to_process {
        println!("   - {}.topic.yml", topic.name);
    }

    // Create partial semantic layer with only changed items
    let partial_semantic_layer = crate::SemanticLayer {
        views: views_to_process,
        topics: if topics_to_process.is_empty() {
            None
        } else {
            Some(topics_to_process)
        },
        metadata: None,
    };

    // Translate to CubeJS format
    let translator = CubeJSTranslator::new(target_dir.clone())?;
    let cube_semantic_layer = translator
        .translate(&partial_semantic_layer, databases.clone())
        .await?;

    // Save to files (writes individual files, no directory clear)
    println!("💾 Saving to cube directory...");
    translator.save_to_files(&cube_semantic_layer).await?;
    ...
    // Scan all current files and compute hashes
    let views_dir = semantic_dir.join("views");
    let topics_dir = semantic_dir.join("topics");
    ...
    if views_dir.exists() {
        for entry in std::fs::read_dir(&views_dir).map_err(|e| {
            ...
            if path.is_file() && path.extension().map(|e| e == "yml").unwrap_or(false) {
                let hash = crate::hash_file(&path)?;
                let relative = path
                    .strip_prefix(semantic_dir.parent().unwrap_or(&semantic_dir))
                    .unwrap_or(&path)
                    .to_string_lossy()
                    .to_string();
                manifest.add_file_hash(&relative, hash);

                // Output mapping: source -> [generated file]
                if let Some(stem) = path.file_stem() {
                    let view_name = stem.to_string_lossy().replace(".view", "");
    ...
    // Compute and set embedding file hashes
    let change_detector = crate::ChangeDetector::new(&semantic_dir, &target_dir);
    let embedding_hashes = change_detector.scan_embedding_files().unwrap_or_default();
    manifest.set_embedding_file_hashes(embedding_hashes);

    // Save manifest
    manifest.save(&target_dir.join(".build_manifest.json"))?;

    println!("🎉 Incremental build completed successfully!");
