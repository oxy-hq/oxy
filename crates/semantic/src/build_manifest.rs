...
pub struct BuildManifest {
    ...
    /// Key: relative path from project root (e.g., "semantics/views/orders.view.yml")
    ...
    /// Key: view name
    /// Value: list of view names that depend on this view
    /// Note: Using BTreeMap for stable iteration order (sorted keys)
...
impl BuildManifest {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Option<Self>, SemanticLayerError> {
        ...
        let manifest: BuildManifest = serde_json::from_str(&content).map_err(|e| {
            SemanticLayerError::ParsingError(format!("Failed to parse manifest: {}", e))
        })?;
