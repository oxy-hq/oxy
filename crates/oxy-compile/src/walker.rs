...
pub fn discover(workspace_root: &Path) -> Result<Vec<DiscoveredFile>, CompileError> {
    ...
    push_glob(workspace_root, "**/*.agentic.yml", FileKind::AgenticAgent, &mut out)?;
    push_glob(workspace_root, "**/*.view.yml", FileKind::SemanticView, &mut out)?;
    push_glob(workspace_root, "**/*.topic.yml", FileKind::SemanticTopic, &mut out)?;
    ...
    push_glob(workspace_root, "**/*.sql", FileKind::VerifiedQuery, &mut out)?;
...
mod tests {
    ...
    fn discover_finds_all_kinds_and_skips_noise() {
        ...
        // config.yml present
        assert!(by_kind.iter().any(|(p, k)| p == "config.yml" && matches!(k, FileKind::Config)));

        // each kind present exactly once (no test/modeling noise)
        assert_eq!(
            by_kind.iter().filter(|(_, k)| matches!(k, FileKind::AgenticAgent)).count(),
            1,
            "exactly one .agentic.yml (test file should be filtered)"
        );
        assert_eq!(
            by_kind.iter().filter(|(_, k)| matches!(k, FileKind::VerifiedQuery)).count(),
            1,
