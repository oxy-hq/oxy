...
use crate::walker::{discover, DiscoveredFile, FileKind, ProcedureKind};
use crate::writer::{
    finalise_revision, insert_compiling_revision, mark_failed, FinaliseInput, FinaliseOutcome,
    RevisionContext,
};
...
pub async fn compile_workspace(
        ...
        .await?
    {
        ...
        if request.promote {
            ...
            .await
            {
                warn!(?e, "idempotent promote failed; falling through to full compile");
            } else {
    ...
    match outcome {
        Ok(o) => Ok(o),
        Err(e) => {
            warn!(?e, "compile failed before finalise; marking revision failed");
...
async fn compile_one(file: &DiscoveredFile) -> Result<Vec<CompiledRow>, FileFailure> {
    ...
    match file.kind {
        ...
        FileKind::AgenticAgent => compile_named_yaml(file, &content, |name, file_path, def| {
            CompiledRow::Agent(CompiledAgent { name, file_path, definition: def })
        }),
        FileKind::SemanticView => compile_named_yaml(file, &content, |name, file_path, def| {
            CompiledRow::View(CompiledView { name, file_path, definition: def })
        }),
        FileKind::SemanticTopic => compile_named_yaml(file, &content, |name, file_path, def| {
            CompiledRow::Topic(CompiledTopic { name, file_path, definition: def })
        }),
        ...
        FileKind::AirwayPipeline => compile_named_yaml(file, &content, |name, file_path, def| {
            CompiledRow::Pipeline(CompiledPipeline { name, file_path, definition: def })
        }),
...
fn compile_monitor_config(file: &DiscoveredFile, content: &str) -> Result<Vec<CompiledRow>, FileFailure> {
    let value = parse_yaml(file, content)?;
    Ok(vec![CompiledRow::MonitorConfig(CompiledMonitorConfig { definition: value })])
}
...
fn compile_app(file: &DiscoveredFile, content: &str) -> Result<Vec<CompiledRow>, FileFailure> {
    ...
    let map = match &value {
        Value::Object(m) => m,
        _ => return Err(shape_err(file, "app YAML must be a mapping at the top level")),
    };
    let name = map
        .get("name")
        .and_then(|v| v.as_str())
...
async fn lookup_revision_by_id(
    ...
    let row = entity::revisions::Entity::find_by_id(revision_id).one(db).await?;
    Ok(row.and_then(|r| {
        if r.status != "ready" {
            return None;
        }
...
mod tests {
    use super::*;
    use crate::walker::{discover, FileKind};
    use std::fs;
    ...
    async fn compile_one_app_extracts_name_and_published() {
        ...
        let app_file = files.iter().find(|f| matches!(f.kind, FileKind::App)).unwrap();
    ...
    async fn compile_one_yaml_failure_returns_structured_error() {
        let dir = TempDir::new().unwrap();
        let root = dir.path();
        write(root, "agents/broken.agentic.yml", "this: is: invalid: yaml:\n");
        let files = discover(root).unwrap();
        let file = files
            .iter()
    ...
    fn derive_name_from_path_strips_known_suffixes() {
        assert_eq!(derive_name_from_path("agents/foo.agentic.yml"), "agents/foo");
