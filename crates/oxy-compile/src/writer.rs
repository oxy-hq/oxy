...
pub async fn finalise_revision(
        ...
        Err(e) if is_unique_violation_for_idempotency(&e) => {
            ...
            return match lookup_ready_winner(db, ctx.workspace_id, &ctx.git_sha).await?
            {
                Some(winner) => {
                    ...
                    if input.promote {
                        // Best-effort promote the winner — if the
                        // workspace already points at a newer revision
                        // the conditional UPDATE in promote_revision
                        // will turn this into a no-op.
                        let _ = promote_existing(db, ctx.workspace_id, winner).await;
                    }
                    Ok(FinaliseOutcome::SupersededBy { revision_id: winner })
                }
    ...
    if input.promote
        && matches!(input.status, RevisionStatus::Ready)
        && input.kind == "main"
    {
...
fn is_unique_violation_for_idempotency(err: &sea_orm::DbErr) -> bool {
    err.to_string().contains("idx_revisions_idempotent_ready_main")
}
...
pub async fn mark_failed(
    ...
    let row = entity::revisions::ActiveModel {
        ...
        finished_at: Set(Some(finished_at.fixed_offset())),
        error_summary: Set(Some(
            serde_json::json!({ "fatal": error }),
        )),
        ..Default::default()
    };
    if let Err(e) = row.update(db).await {
...
async fn write_compiled_rows(
    ...
    for row in rows {
        match row {
            CompiledRow::Config(c) => configs.push(entity::workspace_compiled_configs::ActiveModel {
                revision_id: Set(revision_id),
            ...
            CompiledRow::Procedure(p) => procedures.push(entity::procedure_definitions::ActiveModel {
                revision_id: Set(revision_id),
            ...
            CompiledRow::Reference(r) => references.push(entity::compiled_references::ActiveModel {
                revision_id: Set(revision_id),
    ...
    if !agents.is_empty() {
        entity::agent_definitions::Entity::insert_many(agents).exec(txn).await?;
    }
    if !views.is_empty() {
        entity::semantic_views::Entity::insert_many(views).exec(txn).await?;
    }
    if !topics.is_empty() {
        entity::semantic_topics::Entity::insert_many(topics).exec(txn).await?;
    }
    if !apps.is_empty() {
        entity::app_definitions::Entity::insert_many(apps).exec(txn).await?;
    }
    if !procedures.is_empty() {
        entity::procedure_definitions::Entity::insert_many(procedures).exec(txn).await?;
    }
    if !verified.is_empty() {
        entity::verified_queries::Entity::insert_many(verified).exec(txn).await?;
    }
    if !pipelines.is_empty() {
        entity::airway_pipelines::Entity::insert_many(pipelines).exec(txn).await?;
    }
    if !references.is_empty() {
        entity::compiled_references::Entity::insert_many(references).exec(txn).await?;
    }
    if !monitor_cfgs.is_empty() {
        entity::monitor_configs::Entity::insert_many(monitor_cfgs).exec(txn).await?;
    }
...
async fn prepare_blob_keys(
    ...
    let mut tasks: Vec<std::pin::Pin<Box<dyn std::future::Future<Output = ((crate::blob_store::BlobKind, String), Option<String>)> + Send>>> =
        Vec::new();
    for row in rows {
        match row {
            CompiledRow::View(v) => {
