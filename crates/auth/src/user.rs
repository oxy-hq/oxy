...
impl UserService {
    /// Email address used for the built-in local guest user (no-auth local mode).
    /// This user is always granted Admin role so local installs work out of the box.
    const LOCAL_GUEST_EMAIL: &'static str = "<local-user@example.com>";
    ...
    pub async fn get_or_create_user(identity: &Identity) -> Result<AuthenticatedUser, OxyError> {
            ...
            .map_err(|e| OxyError::DBError(format!("Failed to query user: {e}")))?
        {
            ...
            if existing_user.email == Self::LOCAL_GUEST_EMAIL
        ...
        let initial_role = if identity.email == Self::LOCAL_GUEST_EMAIL {
            UserRole::Admin
        } else {
            UserRole::Member
        };
