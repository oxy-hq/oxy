...
use crate::{config::model::FlashTextSourceType, errors::OxyError};
...
impl Anonymizer for FlashTextAnonymizer {
    fn anonymize(
        ...
        text: &str,
        items: Option<HashMap<String, String>>,
    ) -> Result<(String, HashMap<String, String>), OxyError> {
        self.replace_keywords(text, items)
            .map_err(|err| OxyError::AnonymizerError(format!("Failed to anonymize text: {}", err)))
    }
}
