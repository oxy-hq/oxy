...
use crate::{
    ...
    agent::builders::fsm::{
        config::{
            AgenticConfig, ErrorTriage, RevisePlan, Transition, TransitionMode,
            TransitionObjective, TriggerType,
        },
        control::{
...
impl TriggerBuilder for MachineContext {
    ...
    async fn build_viz_trigger(
        ...
        let openai_adapter =
            OpenAIAdapter::from_config(execution_context.project.clone(), model_ref).await?;
        Ok(Box::new(crate::agent::builders::fsm::viz::GenerateViz::<
            MachineContext,
        >::new(
            objective, openai_adapter, viz_config.clone()
        )))
    }
