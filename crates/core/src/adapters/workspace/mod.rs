...
///
/// - Nil UUID (local dev): uses `resolve_local_workspace_path()` to find config.yml
/// - Non-nil UUID (cloud): fetches project from DB and returns git repo path for active branch
pub async fn resolve_workspace_path(project_id: Uuid) -> Result<PathBuf, OxyError> {
    if project_id.is_nil() {
        ...
        let project = entity::prelude::Workspaces::find_by_id(project_id)
            .one(&conn)
            .await
