...
use crate::{
    ...
    config::{
        constants::{RETRIEVAL_DEFAULT_INCLUSION_RADIUS, RETRIEVAL_EXCLUSION_BUFFER_MULTIPLIER},
...
impl EmbeddingManager {
    ...
    pub(super) async fn embed(&self, documents: &Vec<Document>) -> Result<(), OxyError> {
        ...
        for doc in documents.iter() {
            let inclusions = self
                .embed_retrieval_content(&doc.retrieval_inclusions)
                .await?;
            all_retrieval_inclusions.push(inclusions.clone());

            let exclusions = self
                .embed_retrieval_content(&doc.retrieval_exclusions)
                .await?;
            ...
            let inclusion_radius =
                match MathUtils::find_min_distance(&inclusion_midpoint, &exclusion_embeddings) {
        ...
        let retrieval_inclusions_array = SerializationUtils::create_retrieval_content_array(
            &all_retrieval_inclusions,
            self.embedding_config.n_dims,
        )?;
        let retrieval_exclusions_array = SerializationUtils::create_retrieval_content_array(
            &all_retrieval_exclusions,
            self.embedding_config.n_dims,
        )?;
        let inclusion_midpoint_array =
            Arc::new(
        ...
        self.table_manager
            .add_batches(&table, Box::new(batches))
            .await?;
    ...
    async fn embed_retrieval_content(
        ...
        for (content, embedding_data) in retrieval_content_list
            .iter()
            .zip(embeddings_response.data.iter())
        {
