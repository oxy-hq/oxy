...
pub fn build_content_for_llm_retrieval(document: &Document) -> String {
    let inclusions: Vec<String> = document
        .retrieval_inclusions
        .iter()
        .map(|r| r.embedding_content.clone())
        .collect();
    let exclusions: Vec<String> = document
...
pub fn build_inclusion_exclusion_summary(inclusions: &[String], exclusions: &[String]) -> String {
