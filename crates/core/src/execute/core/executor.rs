    ...
    Event: Send + 'static,
{
    ...
    pub async fn entries<I, E, IT>(&mut self, entries: IT) -> Result<(), OxyError>
    where
    ...
    Event: Send + 'static,
{
    ...
    pub fn finish(self) -> Result<(), OxyError> {
    ...
    pub fn eject(self) -> Result<Vec<ContextValue>, OxyError> {
        let lock = Arc::try_unwrap(self.loop_state)
            .map_err(|_| OxyError::RuntimeError("Failed to eject value from loop".to_string()))?;
        let inner = lock.into_inner()?;
    ...
    pub fn eject_results(self) -> Result<Vec<(usize, ContextValue)>, OxyError> {
        let lock = Arc::try_unwrap(self.loop_state)
            .map_err(|_| OxyError::RuntimeError("Failed to eject value from loop".to_string()))?;
        let inner = lock.into_inner()?;
        Ok(inner.result)
    ...
    Event: Send + 'static,
{
        ...
        T: TemplateRegister,
    {
        ...
        propagate_handle
            .await
            .map_err(|err| OxyError::RuntimeError(err.to_string()))?;
        Ok(())
    }
