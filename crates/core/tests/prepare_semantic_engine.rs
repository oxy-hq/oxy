...
pub mod prepare_semantic_engine {
    ...
    #[test]
    fn test_globals_table_reference_rendered_correctly() {
        let mut cmd = setup_command();
        ...
        // Check users view YAML file
        let users_cube_file = semantics_dir.join("model/users.yml");
        assert!(
            users_cube_file.exists(),
            "users.yml cube file should be created"
        );

        let users_content = fs::read_to_string(&users_cube_file).unwrap();
        let users_yaml: serde_yaml::Value = serde_yaml::from_str(&users_content).unwrap();

        // The YAML structure has 'cubes:' as a top-level array
        let cubes = users_yaml
            .get("cubes")
            .expect("users.yml should have 'cubes' key")
            .as_sequence()
            .expect("cubes should be an array");

        let users_cube = &cubes[0];

        // Verify that the globals.tables.users reference has been rendered to "users"
        if let Some(sql_table) = users_cube.get("sql_table") {
            let sql_table_str = sql_table.as_str().unwrap();
            // Should be rendered as "users.csv" (from globals.tables.users = "users")
            assert!(
                sql_table_str.contains("users.csv"),
                "sql_table should contain 'users.csv' from globals reference, got: {}",
                sql_table_str
            );
            // Should NOT contain the template syntax
            assert!(
                !sql_table_str.contains("{{globals"),
                "sql_table should not contain template syntax, got: {}",
                sql_table_str
            );
        } else if let Some(sql) = users_cube.get("sql") {
            let sql_str = sql.as_str().unwrap();
            assert!(
                sql_str.contains("users.csv"),
                "sql should contain 'users.csv' from globals reference"
            );
            assert!(
                !sql_str.contains("{{globals"),
                "sql should not contain template syntax"
            );
        }
