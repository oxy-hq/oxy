...
impl PipelineTaskExecutor {
    ...
    pub(super) async fn execute_workflow(
        ...
        // dispatched as a synthetic sub-workflow with `body: Some(_)` and
        // no on-disk file. Skip `resolve_workflow_yaml` in that case and
        // deserialize the body directly.
        let mut workflow_config: agentic_workflow::WorkflowConfig =
            if let Some(body) = body.as_ref() {
                serde_json::from_value(body.clone())
                    .map_err(|e| format!("failed to parse inline workflow body: {e}"))?
            } else {
                let yaml = workspace
        ...
        // Pre-resolve every nested sub-workflow's tasks so the decider can
        // emit the full recursive DAG in `subrun_started` (the FE relies on
        // `inner_tasks` for nested step breakdown — see `agentic_workflow::
        ...
        agentic_workflow::resolve_subworkflows(&mut workflow_config, &*workspace).await;
        ...
        let yaml_hash = agentic_workflow::hash::canonical_hash(&workflow_config)
            .map_err(|e| format!("failed to hash workflow config: {e}"))?;
        ...
        // Seed durable workflow state. `initial_render_context` lets a
        // synthetic sub-workflow (loop iteration body) start with its
        ...
        // Fold effective variables (workflow declarations + runtime
        ...
        // the declarations come from `WorkflowConfig.variables` and
    ...
    pub(super) async fn execute_workflow_decision(
        ...
        // `WorkflowRunState` from it for the decider — only `.run_id`,
        // `.step_hashes`, and `.results` are read, so the rest can stay
        // default. This replaces the old per-decision `load_workflow_state`
        ...
        let workspace_path: std::path::PathBuf = {
            let workspace: std::sync::Arc<dyn agentic_workflow::WorkspaceContext> =
                self.platform.clone();
        ...
        // Build the export + cache-write plan from the delta + workflow
        ...
        // conditional), delegated workflow steps (execute_sql /
        // semantic_query / …), agent tasks, and sub-workflows — they
        ...
        // to inline tasks (run_workflow_step used to do the write);
        // agent / sub-workflow steps would have failed silently anyway
...
struct StepWriteEntry {
    ...
    export: Option<agentic_workflow::config::TaskExport>,
    ...
    cache: Option<agentic_workflow::config::CacheConfig>,
...
/// Cross-reference `result_delta` with the workflow's task list to
...
fn build_export_plan(
    state: &agentic_workflow::extension::WorkflowRunState,
...
async fn run_export_plan(
    ...
    let workspace: std::sync::Arc<dyn agentic_workflow::WorkspaceContext> = platform;
    for entry in entries {
        if let Some(export) = &entry.export
            && let Err(e) = agentic_workflow::export::write_export(
                workspace.as_ref(),
                &entry.step_name,
                export,
                &entry.result,
        ...
        if let Some(cache) = &entry.cache
            && let Err(e) = agentic_workflow::export::write_cache(
                workspace.as_ref(),
                &entry.step_name,
                cache,
                &entry.result,
...
/// [`WorkflowDecision`] variant came out of the decider for the current
...
///
/// Emitted from `execute_workflow_decision` *before* the variant's own
...
/// `waiting_on_children`.
fn decider_decided_event(d: &agentic_workflow::WorkflowDecision) -> (String, Value) {
    ...
    let payload = match d {
        D::DelegateStep {
            ...
            let target = match spec {
                ...
                agentic_core::delegation::TaskSpec::Workflow { workflow_ref, .. } => {
                    serde_json::json!({ "kind": "workflow", "workflow_ref": workflow_ref })
                }
                agentic_core::delegation::TaskSpec::WorkflowStep { .. } => {
        ...
        D::DelegateParallel {
            ...
            let targets: Vec<&'static str> = items
                ...
                .map(|it| match &it.target {
                    ...
                    DelegationTarget::Workflow { .. } => "workflow",
                })
                .collect();
            serde_json::json!({
...
fn decision_terminal(
    ...
    match d {
        ...
        D::Fail { error, .. } => DecisionTerminal::FailWorkflow {
            error: error.clone(),
        },
...
fn spec_to_delegation_parts(
    ...
    match spec {
        TaskSpec::Agent {
            ...
            // envelope, the workflow's per-step `extra` payload
...
async fn build_prior_snapshot(
    ...
    String,
> {
    let Some(prior) = agentic_workflow::extension::load_workflow_state(db, prior_run_id)
        .await
        .map_err(|e| format!("load prior workflow state: {e}"))?
    else {
        return Ok((
            std::collections::HashMap::new(),
            std::collections::HashMap::new(),
...
/// Build a minimal `WorkflowRunState` to hand to the decider as `prior_state`.
///
...
) -> agentic_workflow::extension::WorkflowRunState {
    agentic_workflow::extension::WorkflowRunState {
        run_id,
        workflow: agentic_workflow::WorkflowConfig {
            name: String::new(),
            tasks: vec![],
...
mod tests {
    ...
    /// Regression: the workflow→coordinator delegation hop must
