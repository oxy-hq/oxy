//! Shared start/snapshot helpers for workflow runs.
//!
//! HTTP, CLI, MCP, and eval all need the same primitive: "create an
//! agentic_run, enqueue a `TaskSpec::Workflow`, then either watch events
...
//!
//! [`start_workflow_run`] only seeds the DB — it does not run anything.
//! The HTTP handler pairs it with [`spawn_workflow_run_drive`], which
...
use agentic_workflow::extension::WorkflowRunState;
use agentic_workflow::{
    WorkflowConfig, WorkflowDecider, WorkflowDecision, WorkspaceContext, run_workflow_step,
};
...
/// Inputs for [`start_workflow_run`].
#[derive(Debug, Clone, Deserialize)]
pub struct StartWorkflowRequest {
    /// Path to a `.workflow.yml` / `.procedure.yml` / `.automation.yml`,
    /// relative to the workspace root.
    pub workflow_ref: String,
    /// Optional variables for the workflow's render context.
    #[serde(default)]
    ...
    /// via `agentic_runs.thread_id`, which lets the chat-thread workflow
...
impl StartWorkflowRequest {
    ...
    ///
    /// Run by [`start_workflow_run`] on entry. Returning
    /// [`WorkflowRunError::InvalidInput`] surfaces a clear 400 to the HTTP
    ...
    pub fn validate(&self) -> Result<(), WorkflowRunError> {
        validate_workflow_ref(&self.workflow_ref)?;
        if let Some(ref steps) = self.invalidate_steps
            && steps.len() > MAX_INVALIDATE_STEPS
        {
            return Err(WorkflowRunError::InvalidInput(format!(
        ...
        // workflow body isn't parsed yet; the decider silently no-ops
...
fn validate_workflow_ref(workflow_ref: &str) -> Result<(), WorkflowRunError> {
...
/// Snapshot of a workflow run: the platform-side run record plus the
/// agentic-workflow state row, flattened for serialization.
...
/// Errors from [`start_workflow_run`] / [`get_workflow_snapshot`] /
/// [`run_inline_workflow`].
...
pub enum WorkflowRunError {
    ...
    #[error("run not found")]
    NotFound,
    #[error("inline workflow: {0}")]
    Inline(String),
    /// Client-supplied [`StartWorkflowRequest`] failed validation. HTTP
...
/// Maximum entries allowed in [`StartWorkflowRequest::invalidate_steps`].
///
/// Workflows realistically have a few dozen steps; capping the list keeps a
...
/// Picked generously so a legitimate workflow can't trip it.
pub const MAX_INVALIDATE_STEPS: usize = 256;

/// Maximum total entries across all steps in [`StartWorkflowRequest::invalidate_iterations`].
///
/// Workflow authors might fan out a few dozen iterations per loop; a
...
/// synchronously inside the inline workflow runner.
///
...
/// agent runtime.
///
/// Used by the inline runner only when a workflow contains agent fan-out
...
pub trait InlineAgentRunner: Send + Sync {
    ...
    ///
    /// `agent_ref` matches the `agent_ref` field in the workflow's
...
/// [`TaskSpec::Workflow`] for the coordinator to claim.
///
...
pub async fn start_workflow_run(
    ...
    let thread_uuid = match request.thread_id.as_deref() {
        Some(s) => Some(
            Uuid::parse_str(s)
                .map_err(|e| WorkflowRunError::InvalidInput(format!("thread_id {s:?}: {e}")))?,
        ),
        None => None,
    };
...
/// Spawn a coordinator + worker pair that drives a queued workflow run
/// to completion.
///
/// Pairs with [`start_workflow_run`]. The coordinator manages the task
...
/// outcomes for unregistered task ids), which manifests as a workflow
...
/// Drive a queued workflow run.
///
/// `builder_bridges` is required for workflows that delegate to the
...
/// sweeper is the historical example), and any workflow that does
/// invoke the builder from that path will fail loudly.
pub fn spawn_workflow_run_drive(
    db: DatabaseConnection,
    ...
    // `WorkflowDecision`. `register_root` (vs. `submit_root`) does
    // not re-enqueue; the queue entry already exists from
    // `start_workflow_run`'s `crud::enqueue_task` call.
    ...
    tracing::debug!(
        target: "workflow_run",
        run_id = %run_id,
        "spawning coordinator + worker for queued workflow run"
    );
...
pub async fn list_workflow_runs(
    ...
    let stmt = Statement::from_sql_and_values(
        ...
        "#,
        [
            agentic_workflow::SOURCE_TYPE.into(),
            workflow_ref.into(),
            (limit as i64).into(),
        ],
    );
...
/// Fetch a [`WorkflowRunSnapshot`] for the given run id.
///
/// Returns [`WorkflowRunError::NotFound`] when the platform-side run record
/// is missing entirely. The agentic-workflow extension state row may legally
...
pub async fn get_workflow_snapshot(
    ...
    let wf_state = agentic_workflow::extension::load_workflow_state(db, run_id).await?;

    Ok(match wf_state {
        Some(s) => WorkflowRunSnapshot {
...
/// Run a workflow synchronously, in-process, without involving the
/// coordinator / worker queue or any DB row.
///
/// Intended for callers that already have a typed [`WorkflowConfig`]
...
///
/// Drives [`WorkflowDecider`] in a loop:
...
async fn run_inline_workflow_internal(
    workspace: &dyn WorkspaceContext,
    workflow: WorkflowConfig,
    variables: Option<Value>,
    agent_runner: Option<&dyn InlineAgentRunner>,
    ...
    let workflow_yaml_hash = agentic_workflow::hash::canonical_hash(&workflow)
        .map_err(|e| WorkflowRunError::Inline(format!("hash workflow config: {e}")))?;
    ...
    for _ in 0..1024 {
        ...
        state = new_state;
        match decision {
            WorkflowDecision::StepExecutedInline { .. } => continue,
            WorkflowDecision::Complete { .. } => return Ok(state.results),
            WorkflowDecision::Fail { error, .. } => return Err(WorkflowRunError::Inline(error)),
            WorkflowDecision::WaitForMoreChildren => {
                return Err(WorkflowRunError::Inline(
...
///
/// **Loops** (`DelegationTarget::Workflow { workflow_ref = "__workflow_step__" }`)
...
async fn run_parallel_inline(
    ...
    for (idx, item) in items.iter().enumerate() {
        let entry = match &item.target {
            DelegationTarget::Workflow { workflow_ref } if workflow_ref == "__workflow_step__" => {
                run_loop_iteration_inline(workspace, &item.context, agent_runner).await?
            }
            DelegationTarget::Workflow { workflow_ref } => {
                return Err(WorkflowRunError::Inline(format!(
                    "inline parallel: sub-workflow target '{workflow_ref}' is unsupported"
                )));
            }
                ...
                let runner = agent_runner.ok_or_else(|| {
                    WorkflowRunError::Inline(
                        "inline parallel: consistency runs (agent fan-out) require an \
                         InlineAgentRunner; pass one via run_inline_workflow_with"
                            .into(),
                    )
                })?;
                let answer = runner
    ...
    serde_json::to_string(&aggregated)
        .map_err(|e| WorkflowRunError::Inline(format!("aggregate: {e}")))
}
...
async fn run_loop_iteration_inline(
    ...
    let step_config = context
        .get("step_config")
        .ok_or_else(|| WorkflowRunError::Inline("loop item: missing step_config".into()))?;
    // step_config is `{name, tasks}` — a sub-workflow body. Round-trip via
    // JSON onto the typed config; `WorkflowConfig` ignores unknown fields,
    // so the synthetic `name` field doesn't trip parsing.
    let sub_workflow: WorkflowConfig = serde_json::from_value(step_config.clone())
        .map_err(|e| WorkflowRunError::Inline(format!("loop item: parse sub-workflow: {e}")))?;
    ...
    let answer = serde_json::to_string(&sub_results)
        .map_err(|e| WorkflowRunError::Inline(format!("loop item: serialize answer: {e}")))?;
...
mod tests {
    ...
    fn base_request(workflow_ref: &str) -> StartWorkflowRequest {
    ...
    fn assert_invalid(req: &StartWorkflowRequest, needle: &str) {
        match req.validate() {
            Err(WorkflowRunError::InvalidInput(msg)) => assert!(
                msg.contains(needle),
    ...
    #[test]
    fn accepts_relative_workflow_ref() {
        for r in [
            "foo.workflow.yml",
    ...
    fn rejects_oversized_invalidate_steps() {
        let mut req = base_request("foo.workflow.yml");
