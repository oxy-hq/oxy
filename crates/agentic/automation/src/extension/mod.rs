//! Automation state extension table: per-run Temporal-style workflow state.
//!
//! The `entity` and `crud` submodules are crate-private — external consumers
//! use the [`AutomationRunState`] DTO and the facade functions below.
...
///
/// Persisted in `agentic_workflow_state`. A `AutomationDecision` task loads
/// this, calls `AutomationDecider::decide()`, updates state, and exits — no
/// in-memory channels survive a crash.
/// Back-compat alias (Procedures/Workflows → Automations rename). External
/// callers still import `extension::WorkflowRunState`.
pub type WorkflowRunState = AutomationRunState;

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AutomationRunState {
    pub run_id: String,
    pub workflow: AutomationConfig,
    ...
    /// `TaskSpec::Automation { initial_render_context: Some(_), … }`
...
impl TryFrom<entity::Model> for AutomationRunState {
    type Error = DbErr;

    fn try_from(m: entity::Model) -> Result<Self, DbErr> {
        let workflow: AutomationConfig =
...
pub async fn insert_automation_state(
...
pub async fn load_automation_state(
    db: &DatabaseConnection,
    run_id: &str,
) -> Result<Option<AutomationRunState>, DbErr> {
    match crud::load_state(db, run_id).await? {
        Some(model) => Ok(Some(AutomationRunState::try_from(model)?)),
        None => Ok(None),
    }
}
...
pub async fn update_automation_state(
    db: &DatabaseConnection,
    state: &AutomationRunState,
) -> Result<bool, DbErr> {
