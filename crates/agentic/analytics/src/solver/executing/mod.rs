...
pub(super) enum SqlGenOutcome {
    ...
    /// generated SQL string; downstream workflow steps consume it via
    /// the workflow `cache:` block.
    ...
    /// engine, procedure delegation). Workflow authors hit this when
...
impl AnalyticsSolver {
    ...
    pub(super) async fn terminate_with_sql_if_enabled(
        ...
        // Vendor and procedure paths produce no portable SQL string —
        // reject so the workflow author sees a clear error rather than
        // an empty cache file.
        match &solution.solution_source {
            ...
            SolutionSource::Procedure { file_path } => {
                return SqlGenOutcome::IncompatiblePath {
                    reason: format!(
                        "procedure delegation ({}) produces no SQL until the \
                         child workflow runs; incompatible with \
                         `output: {{ mode: sql }}`",
                        file_path.display()
                    ),
                };
            ...
            |solver: &mut AnalyticsSolver,
             ...
             _memory: &SessionMemory<AnalyticsDomain>| {
                Box::pin(async move {
                    ...
                    // ── SQL-generation mode ────────────────────────────────────
                    // Workflows that set `output: { mode: sql }` on the
                    ...
                    // check first. Vendor/procedure paths are
                    ...
                    match solver.terminate_with_sql_if_enabled(&solution).await {
                        ...
                        SqlGenOutcome::IncompatiblePath { reason } => {
                            // Hard fail — the workflow YAML is
                            // misconfigured (vendor / procedure solution
                            ...
                            // is reasonable here — the workflow author
...
#[cfg(test)]
fn procedure_output_to_result(output: SubrunOutput) -> AnalyticsResult {
