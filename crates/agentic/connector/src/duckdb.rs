...
fn epoch_ts_to_iso(unit: &TimeUnit, value: i64) -> String {
    let secs = match unit {
...
fn duckdb_to_cell(v: Value) -> CellValue {
    match v {
...
fn describe_table(conn: &Connection, table: &str) -> Result<Vec<(String, String)>, duckdb::Error> {
...
/// Parse the string representation of a min/max value returned by `SUMMARIZE`
/// into a [`CellValue`], using `col_type` to decide whether to attempt numeric
/// parsing.  Returns `None` for empty strings or unparsable values.
fn parse_summarize_cell(s: &str, col_type: &str) -> Option<CellValue> {
    if s.is_empty() {
        return None;
    }
...
/// `None` for NULL and types that have no meaningful scalar representation.
fn duckdb_to_cell_opt(v: Value) -> Option<CellValue> {
    match v {
        Value::Null => None,
