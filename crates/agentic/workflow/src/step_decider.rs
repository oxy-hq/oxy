...
impl WorkflowDecider {
    ...
    pub async fn decide(
        ...
        if let Some(child) = pending_child_answer {
            ...
            // render context, advance — *or* halt the workflow on failure.
            ...
            // On failure, halt the workflow. Without this `Fail` return,
            ...
            // frontend's "Workflow failed" status flips immediately rather
        ...
        // full nested task DAG (loops + pre-resolved sub-workflows) so
        ...
        // sub-workflow children get pre-resolved before the run starts.
        if step_index == 0 {
        ...
        let current_hash = match compute_step_hash(&StepHashInputs {
            ...
            loop_idx: None,
            // Sub-workflow's child config hash is wired in a later patch —
...
fn resolve_loop_values(values: &Value, render_context: &Value) -> Result<Vec<Value>, String> {
    ...
    let env = crate::render::workflow_env_strict();
    let expr = env
...
mod final_answer_tests {
    use super::*;
    use crate::config::{FormatterConfig, TaskConfig, TaskType, WorkflowConfig};
    use std::collections::HashMap;

    fn wf(task_names: &[&str]) -> WorkflowConfig {
        WorkflowConfig {
            name: "wf".into(),
    ...
    fn state_with(workflow: WorkflowConfig, results: Vec<(&str, Value)>) -> WorkflowRunState {
...
mod iteration_cache_tests {
    ...
    use crate::config::{LoopConfig, TaskConfig, TaskType, WorkflowConfig};
    use serde_json::{Value, json};

    fn workflow_with_loop(values: Value) -> WorkflowConfig {
        WorkflowConfig {
            name: "wf".into(),
    ...
    fn fold_builds_iterations_map_for_loop_step() {
        let wf = workflow_with_loop(json!([10, 20, 30]));
...
mod cache_tests {
    use super::*;
    use crate::config::{FormatterConfig, TaskConfig, WorkflowConfig};
    use std::collections::HashMap;

    fn formatter_workflow(name: &str, template: &str) -> WorkflowConfig {
        WorkflowConfig {
            name: "test-wf".into(),
            description: String::new(),
            variables: None,
    ...
    async fn cache_hit_reuses_prior_result() {
        let wf = formatter_workflow("greet", "hi");
        ...
        let event_types: Vec<&str> = match &decision {
            WorkflowDecision::StepExecutedInline { emitted_events, .. } => {
    ...
    async fn iteration_overrides_bypass_step_level_cache_hit() {
        ...
        if let WorkflowDecision::StepExecutedInline { emitted_events, .. } = &decision {
            assert!(
                !emitted_events
                    .iter()
    ...
    async fn file_cache_hit_short_circuits_step() {
        ...
        let events = match &decision {
            WorkflowDecision::StepExecutedInline { emitted_events, .. } => emitted_events,
    ...
    async fn config_change_misses_cache() {
        let prior_wf = formatter_workflow("greet", "hello");
        ...
        let new_wf = formatter_workflow("greet", "HOLA");
