...
//! - **`variables`** — workflow-level variables (CLI args, sub-workflow
//!   inputs).
//! - **`loop_idx`** — the iteration index, when this step is one iteration of
//!   a `loop_sequential` parent.
//! - **`sub_workflow_yaml_hash`** — for `SubWorkflow` steps, the canonical
//!   hash of the child workflow config. Editing the child must invalidate the
//!   parent step.
...
pub struct StepHashInputs<'a> {
    ...
    /// For `SubWorkflow` steps only — canonical hash of the child config.
...
fn code_version_for(t: &crate::config::TaskType) -> u32 {
    ...
    match t {
        ...
        SubWorkflow(_) => SUB_WORKFLOW_CODE_VERSION,
...
mod tests {
    ...
    #[test]
    fn sub_workflow_hash_propagates() {
        // Same parent step config; child workflow hash differs → step hash differs.
        let step = TaskConfig {
            name: "child".into(),
            task_type: TaskType::SubWorkflow(crate::config::SubWorkflowConfig {
