/**
 * Convert the new agentic-workflow event stream into the same nested
 * `LogItem[]` tree the legacy procedure UI expected. Each step becomes a
...
import type { SubrunStepInfo, WorkflowEvent } from "@/services/api/agenticWorkflows";
...
type StepRow = {
   ...
   * Container types (`loop_sequential`, `workflow`) carry their nested
   * task DAG so iteration / sub-workflow output can be decomposed by
 ...
 */
export function useLogItems(events: WorkflowEvent[]): LogItem[] {
  return useMemo(() => buildLogItems(events), [events]);
}

/**
 * Stateless variant of `useLogItems` for callers outside React (e.g. the
 * chat-thread workflow runner, which keeps events in a closure and writes
 * the tree into a zustand store on each SSE message).
 */
export function buildLogItems(events: WorkflowEvent[]): LogItem[] {
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "subrun_completed": {
        ...
        // "Running" forever when the workflow halted mid-step. Distinguish
        // the failure point from downstream steps that never ran:
        //   running → `failed` (was actively in flight when the workflow died)
        ...
        if (!event.payload.success) {
          for (const row of stepsByName.values()) {
            if (row.status === "running") {
              ...
              row.timestamp = nowIso();
              if (!row.errorMessage) {
                row.errorMessage = "Workflow halted while this step was running";
                row.body.push({
...
// legacy block→LogItem mapping in `useWorkflowRun.ts`'s `logSelector`.
...
function renderStepOutput(
  ...
  switch (taskType) {
    ...
    case "workflow":
      return renderSubWorkflowOutput(output, ts, innerTasks);
    default:
...
function renderIterationBody(
  ...
  // like a mini-procedure run instead of a wall of JSON.
...
/**
 * Render a sub-workflow step's output.
 *
 * The output is the child workflow's `build_final_answer` —
 ...
 * rendering.
 */
function renderSubWorkflowOutput(
  output: unknown,
  ts: string,
  innerTasks?: SubrunStepInfo[]
): LogItem[] {
