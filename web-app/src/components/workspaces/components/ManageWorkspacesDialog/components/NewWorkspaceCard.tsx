...
export function NewWorkspaceCard({ index, onClick }: Props) {
  return (
    <div
      className='fade-in slide-in-from-bottom-2 h-full w-full animate-in fill-mode-both duration-300'
      style={{ animationDelay: `${index * 60}ms` }}
    >
      <button
        type='button'
        onClick={onClick}
        className='group flex h-full min-h-36 w-full flex-col items-center justify-center gap-2.5 rounded-xl border border-border/50 border-dashed bg-card transition-all hover:border-primary/40 hover:bg-primary/[0.02]'
      >
