import type { ReactNode } from "react";
import { Spinner } from "@/components/ui/shadcn/spinner";

import { cn } from "@/libs/shadcn/utils";

export type NodeStatus = "loading" | "success" | "error" | "initial";

export type NodeStatusVariant = "overlay" | "border";

export type NodeStatusIndicatorProps = {
  status?: NodeStatus;
  variant?: NodeStatusVariant;
  children: ReactNode;
};

export const SpinnerLoadingIndicator = ({ children }: { children: ReactNode }) => {
  return (
    <div className='relative -z-1'>
      <StatusBorder className='border-info/40'>{children}</StatusBorder>

      <div className='absolute inset-0 z-50 rounded-[7px] bg-background/50 backdrop-blur-sm' />
      <div className='absolute inset-0 z-50'>
        <span className='absolute top-[calc(50%-1.25rem)] left-[calc(50%-1.25rem)] inline-block h-10 w-10 animate-ping rounded-full bg-info/20' />

        <Spinner className='absolute top-[calc(50%-0.75rem)] left-[calc(50%-0.75rem)] size-6 text-info' />
      </div>
    </div>
  );
};

export const BorderLoadingIndicator = ({ children }: { children: ReactNode }) => {
  return (
    <>
      <div className='absolute -top-[1px] -left-[1px] -z-1 h-[calc(100%+2px)] w-[calc(100%+2px)]'>
        <style>
          {`
        @keyframes spin {
          from { transform: translate(-50%, -50%) rotate(0deg); }
          to { transform: translate(-50%, -50%) rotate(360deg); }
        }
        .spinner {
          animation: spin 2s linear infinite;
          position: absolute;
          left: 50%;
          top: 50%;
          aspect-ratio: 1;
          transform-origin: center;
        }
        @container (min-aspect-ratio: 1/1) {
          .spinner { width: 140%; }
        }
        @container (max-aspect-ratio: 1/1) {
          .spinner { height: 140%; }
        }
      `}
        </style>
        <div className='absolute inset-0 overflow-hidden rounded-[10px] [container-type:size]'>
          <div className='spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,_var(--special)_0deg,_transparent_360deg)]' />
        </div>
      </div>
      {children}
    </>
  );
};

const StatusBorder = ({ children, className }: { children: ReactNode; className?: string }) => {
  return (
    <>
      <div
        className={cn(
          "absolute -top-[1px] -left-[1px] -z-1 h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-[10px] border-2",
          className
        )}
        data-testid='workflow-node-status-border'
      />
      {children}
    </>
  );
};

export const NodeStatusIndicator = ({
  status,
  variant = "border",
  children
}: NodeStatusIndicatorProps) => {
  switch (status) {
    case "loading":
      switch (variant) {
        case "overlay":
          return <SpinnerLoadingIndicator>{children}</SpinnerLoadingIndicator>;
        case "border":
          return <BorderLoadingIndicator>{children}</BorderLoadingIndicator>;
        default:
          return <>{children}</>;
      }
    case "success":
      return <StatusBorder className='border-success'>{children}</StatusBorder>;
    case "error":
      return <StatusBorder className='border-destructive'>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
