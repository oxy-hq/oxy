...
import type { GitHubNamespace, GitHubRepository } from "@/types/github";
...
export default function GitHubImportStep({
  ...
  const [selectedGitNamespace, setSelectedGitNamespace] = useState<GitHubNamespace | null>(
    initialData?.namespace || null
  );

  const [selectedRepository, setSelectedRepository] = useState<GitHubRepository | null>(
    initialData?.repository || null
  );

  const [selectedBranch, setSelectedBranch] = useState<string>(initialData?.branch || "");

  const { data: gitNamespaces = [], isLoading: isLoadingNamespaces } = useGitHubNamespaces();

  const { data: repositories = [], isLoading: isLoadingRepos } = useGitHubRepositoriesWithApp(
    selectedGitNamespace?.id || ""
  );

  const { data: branches = [], isLoading: isLoadingBranches } = useGitHubBranchesWithApp(
    selectedGitNamespace?.id || "",
    selectedRepository?.full_name || ""
  );

  const isLoading = isLoadingNamespaces || isLoadingRepos || isLoadingBranches || isCreating;

  const isNextDisabled = !selectedGitNamespace || !selectedRepository || isLoading;
  ...
  const renderRepositoriesSection = () => {
    ...
    if (repositories.length === 0) {
      return (
        <div className='rounded border p-2 text-muted-foreground text-sm'>
          No repositories found. Make sure the GitHub App has access to repositories.
        </div>
      );
    }
    ...
    return (
      <Combobox
        items={repositories.map((repo: GitHubRepository) => ({
          ...
          searchText: `${repo.full_name} ${repo.name} ${repo.description || ""}`.toLowerCase()
        }))}
        value={selectedRepository?.id.toString() || ""}
        onValueChange={(value) => {
          const repo = repositories.find((r: GitHubRepository) => r.id.toString() === value);
          ...
          <div className='flex w-full items-center justify-between'>
            <div className='flex items-center gap-2'>
              ...
              <div>
                ...
                {repositories.find((r: GitHubRepository) => r.id.toString() === item.value)
                  ?.description && (
  ...
  return (
    <div className='space-y-6'>
      <div>
        <h2 className='mb-2 font-bold text-2xl'>Import from GitHub</h2>
      ...
      <div className='space-y-4'>
        <div className='space-y-6'>
          ...
          {selectedRepository && <SelectedRepositoryDisplay repository={selectedRepository} />}
          ...
          <Button
            ...
            onClick={() =>
              onNext({
                ...
                branch: selectedBranch || selectedRepository?.default_branch || ""
              })
            }
