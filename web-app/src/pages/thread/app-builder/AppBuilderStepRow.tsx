...
function getToolDisplay(item: ArtifactItem): ToolDisplay {
  ...
  switch (item.toolName) {
    ...
    case "preview_data": {
      ...
      return { Icon: Table2, label: "Preview Table", preview: `${table} (${limit} rows)` };
    }
    case "get_column_values": {
      const table = typeof input?.table === "string" ? input.table : "?";
      const col = typeof input?.column === "string" ? input.column : "?";
      return { Icon: Layers, label: "Column Values", preview: `${table}.${col}` };
    }
    case "get_column_range": {
      const table = typeof input?.table === "string" ? input.table : "?";
      const col = typeof input?.column === "string" ? input.column : "?";
      return { Icon: BarChart2, label: "Column Range", preview: `${table}.${col}` };
    }
    case "get_join_path": {
    ...
    case "count_rows": {
      const table = typeof input?.table === "string" ? input.table : "?";
      const filter = typeof input?.filter === "string" ? ` WHERE ${input.filter}` : "";
      return { Icon: Database, label: "Count Rows", preview: trunc(`${table}${filter}`) };
    }
    case "sample_column": {
      const table = typeof input?.table === "string" ? input.table : "?";
      const col = typeof input?.column === "string" ? input.column : "?";
...
const ArtifactChild = ({
  ...
  item: ArtifactItem;
  onSelect: (item: AppBuilderSelectableItem) => void;
}) => {
...
interface AppBuilderStepRowProps {
  step: AnalyticsStep;
  onSelectArtifact: (item: AppBuilderSelectableItem) => void;
}

const AppBuilderStepRow = ({ step, onSelectArtifact }: AppBuilderStepRowProps) => {
        ...
        {step.items.length > 0 && (
          <div className={cn("mt-1 mb-2 ml-8 space-y-1.5 border-l-2 pl-3", colors.border)}>
            {step.items.map((item) => {
              ...
              if (item.kind === "text") return <TextChild key={item.id} item={item} />;
              // procedure items are not rendered in app-builder
              return null;
            })}
          </div>
        )}
...
export default AppBuilderStepRow;
