...
import AppBuilderStepRow from "./AppBuilderStepRow";
          ...
          activeCard.steps.length > 0 ? (
            <div className='space-y-1'>
              {activeCard.steps.map((step) => (
                <AppBuilderStepRow key={step.id} step={step} onSelectArtifact={onSelectArtifact} />
              ))}
            </div>
          ) : (
...
interface AppBuilderReasoningTraceProps {
  events: UiBlock[];
  isRunning: boolean;
  onSelectArtifact: (item: AppBuilderSelectableItem) => void;
}

const AppBuilderReasoningTrace = ({
  events,
  isRunning,
  onSelectArtifact
}: AppBuilderReasoningTraceProps) => {
...
export default AppBuilderReasoningTrace;
