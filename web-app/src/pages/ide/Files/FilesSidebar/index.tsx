...
const FilesSidebar: React.FC<{
  ...
  const fileTree = useMemo(() => {
    const filtered = data?.filter(
      (f) => !IGNORE_FILES_REGEX.some((r) => f.name.match(r)),
    );
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-sidebar-background">
      <SidebarHeader
        title={
          filesSubViewMode === FilesSubViewMode.OBJECTS ? "Objects" : "Files"
        }
      ...
      <div className="px-2 py-2 border-b border-sidebar-border">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 h-7">
            <TabsTrigger value="objects" className="text-xs h-6 gap-1">
              <Layers2 className="h-3 w-3" />
              Objects
            </TabsTrigger>
            <TabsTrigger value="files" className="text-xs h-6 gap-1">
