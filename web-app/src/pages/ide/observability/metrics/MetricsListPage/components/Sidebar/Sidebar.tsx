...
import { SOURCE_TYPE_CONFIG, CONTEXT_TYPE_CONFIG } from "../../constants";
...
export default function Sidebar({ analyticsData, isLoading }: SidebarProps) {
  ...
  const sourceTypeStats = useMemo(() => {
    ...
    const total =
      analyticsData.by_source_type.agent +
      analyticsData.by_source_type.workflow;
    return {
      agent: {
        count: analyticsData.by_source_type.agent,
        percentage:
          total > 0
            ? Math.round((analyticsData.by_source_type.agent / total) * 100)
            : 0,
      },
      workflow: {
        ...
        percentage:
          total > 0
            ? Math.round((analyticsData.by_source_type.workflow / total) * 100)
            : 0,
      },
    };
  }, [analyticsData]);
  ...
  const contextTypeStats = useMemo(() => {
    ...
    return {
      sql: {
        ...
        percentage:
          total > 0
            ? Math.round((analyticsData.by_context_type.sql / total) * 100)
            : 0,
      },
      question: {
        ...
        percentage:
          total > 0
            ? Math.round((analyticsData.by_context_type.question / total) * 100)
            : 0,
      },
      response: {
        ...
        percentage:
          total > 0
            ? Math.round((analyticsData.by_context_type.response / total) * 100)
            : 0,
      },
      semantic: {
        ...
        percentage:
          total > 0
            ? Math.round(
                (analyticsData.by_context_type.semantic_query / total) * 100,
              )
            : 0,
      },
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <SourceDistribution
            stats={contextTypeStats}
            config={CONTEXT_TYPE_CONFIG}
          />
        </CardContent>
      </Card>
