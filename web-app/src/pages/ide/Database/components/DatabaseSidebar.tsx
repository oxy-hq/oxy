...
import { Loader2, Database as DatabaseIcon, Plus } from "lucide-react";
import {
  SidebarContent,
  SidebarGroup,
  SidebarMenu,
        ...
        <SidebarGroup className="pt-2">
          ...
          {!isLoading && databases.length === 0 && (
            <div className="flex flex-col items-center justify-center p-4 text-muted-foreground text-sm">
              <DatabaseIcon className="h-8 w-8 mb-2 opacity-50" />
            ...
            <SidebarMenu className="pb-20">
              {databases.map((database) => (
                <ConnectionItem
                  ...
                  onSelect={() =>
                    setActiveConnection(database.name.toLowerCase())
                  }
                />
              ))}
            </SidebarMenu>
