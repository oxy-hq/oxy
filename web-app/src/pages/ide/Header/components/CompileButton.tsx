...
export function CompileButton() {
  ...
  const button = (
    <Button
      ...
      className={cn(
        "group relative gap-1.5 overflow-hidden pr-2 pl-2 transition-colors duration-200",
...
function StateIcon({ kind, loading }: { kind: ViewKind; loading: boolean }) {
  ...
  if (kind === "fresh") return <CircleCheck className='size-3.5 text-emerald-600/80' />;
  if (kind === "failed") return <TriangleAlert className='size-3.5 text-destructive' />;
