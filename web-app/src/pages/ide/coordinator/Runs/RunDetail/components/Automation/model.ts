...
/**
 * Automation-run derived model. The backend gives us:
 ...
 * inspector. Linear v1 — automations declare `depends_on` in YAML but
 * we render execution order as a straight chain; structural edges are
 * a separate enrichment for when automations grow real fan-outs.
 */
export interface AutomationNode {
  ...
  key: string;
  /** Step name from the automation YAML. */
  name: string;
   ...
   *  For container nodes (loop / sub-automation) the kind reflects the
   *  container itself, not its children. */
  kind: AutomationNodeKind;
  /** Raw `task_type` from the YAML (e.g. `"execute_sql"`,
   *  `"loop_sequential"`, `"automation"`, `"agent"`). Drives container
  ...
  /** When the step delegated to another run (agent / sub-automation),
   ...
   *  (`loop_sequential`, `automation`). Empty for leaf steps. */
  children: AutomationNode[];
}

export type AutomationNodeKind = "sql" | "agent" | "automation" | "loop" | "generic";

/** Container task types from the automation YAML — their `inner_tasks`
...
export interface AutomationModel {
  ...
  criticalNode: AutomationNode | null;
...
const kindForTaskType = (taskType: string): AutomationNodeKind | null => {
  switch (taskType) {
    case "execute_sql":
    case "semantic_query":
    case "omni_query":
...
const detectKindFromEvents = (events: RunEventEntry[]): AutomationNodeKind => {
  ...
  if (hasDelegation) return "automation";
...
const extractQuery = (events: RunEventEntry[]): AutomationNode["query"] | undefined => {
 ...
 *  automation orchestrator emits this once at the start of every run
 ...
 *  slice. Its children are the iteration's own automation tree, built
...
const buildIterationNode = (
  ...
  // Each iteration is itself an automation run — recurse with the inner
  ...
  const innerModel = buildAutomationModel(innerSteps, slice.innerEvents);
 ...
 *  inner automation under a synthetic iteration parent. */
const rekeyNode = (node: AutomationNode, newKey: string): AutomationNode => ({
  ...node,
  key: newKey,
...
/** Convert one `SubrunStepDesc` to a `AutomationNode`, recursing into
...
const buildNode = (
  ...
  const nestedWaterfall =
    !isContainer && (kind === "agent" || kind === "automation")
...
const flattenLeaves = (nodes: AutomationNode[]): AutomationNode[] => {
  const out: AutomationNode[] = [];
  const walk = (ns: AutomationNode[]) => {
    for (const n of ns) {
...
export const buildAutomationModel = (
  ...
  const nodes: AutomationNode[] = tree
    ...
    : steps.map((s, i) => {
        ...
        return {
          ...
          events: stepEvents,
          nestedWaterfall:
            kind === "agent" || kind === "automation" ? buildWaterfall(stepEvents) : undefined,
  ...
  let criticalLeaf: AutomationNode | null = null;
  let maxDuration = 0;
...
const deriveWindow = (
  ...
  const visit = (ns: AutomationNode[]): void => {
    for (const n of ns) {
      if (n.startedAt) {
