...
/**
 * Side panel for the currently selected workflow step. Routes by
 * `node.kind`: SQL gets a query view with first-rows preview, agent /
 * procedure get the existing `Waterfall` embedded against the step's
 * scoped event log, generic falls back to status + error + duration.
 *
 * Reusing `<Waterfall events=…/>` here is the load-bearing move — a
 * workflow that delegates to an analytics agent gets the same
 * phase/LLM/tool/cost render the standalone agent run page uses, with
 * zero duplicate code.
 */
export const StepInspector: React.FC<{ node: WorkflowNode | null }> = ({ node }) => {
  if (!node) {
    return (
  ...
  return (
    <div className='space-y-3 p-4'>
      ...
      {(node.kind === "agent" || node.kind === "procedure") && node.events.length > 0 && (
...
const QueryDetail: React.FC<{
  query: NonNullable<WorkflowNode["query"]>;
}> = ({ query }) => (
  <div className='space-y-2'>
    <div className='flex flex-wrap gap-x-3 gap-y-1 text-muted-foreground text-xs'>
      <span>
