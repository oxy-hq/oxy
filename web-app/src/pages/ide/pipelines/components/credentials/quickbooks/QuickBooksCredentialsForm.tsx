  ...
  return (
    <div className='grid gap-3 rounded-md border border-border p-3'>
      ...
      <div className='grid gap-2 border-border border-t pt-3'>
        {/* Connect writes workspace secrets — WorkspaceAdmin only. Hidden only
          for a KNOWN non-admin; an unloaded role must not blank the button. */}
        {(wsRole === undefined || is.workspaceAdmin) && (
          <Button
            type='button'
            variant='outline'
            onClick={onConnect}
        ...
        {credentials.connected ? (
          ...
          <p className='text-muted-foreground text-xs'>
            Authorize with Intuit to capture the realm id + refresh token automatically. Register
            this Redirect URI in your Intuit app:{" "}
