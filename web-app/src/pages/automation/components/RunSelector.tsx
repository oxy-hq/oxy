...
import { useAutomationRunsForAutomation } from "@/hooks/api/agentic-automations/useAgenticAutomations";

interface Props {
  /** Automation ref the dropdown lists runs for (decoded path, e.g. `automations/foo.automation.yml`). */
...
const RunSelector: React.FC<Props> = ({ automationRef, runId, onSelect }) => {
  const { data: runs, isPending } = useAutomationRunsForAutomation(automationRef);
