/**
 * Per-step run status, scoped to whichever automation tree is rendered
 * under the provider.
 *
 * The legacy `useAutomationRun` zustand store kept this state global —
 ...
 * already produced by `useAutomationRunStream` (no zustand involvement).
 *
 ...
 * new automation page is) the entry wins; when it's not (the legacy
 * `AutomationPreview` still uses the diagram), the legacy path keeps
 * working untouched.
 */
 ...
 * `AutomationPreview` consumer) — the diagram falls back to its own
