...
import { Hammer, Loader2, PlayCircle, RefreshCw } from "lucide-react";
...
import {
  useBackfillUncompiled,
  useCompiles,
  usePromoteCompile,
  useRunCompileNow
} from "@/hooks/api/compiles";
...
const CompileRowItem = ({ row }: { row: CompileRow }) => {
  ...
  // Only a ready main revision that isn't already current is a sensible
  // rollback target.
  const canPromote = row.status === "ready" && row.kind === "main" && !row.is_current_for_workspace;
  const onPromote = () => {
    if (
      !window.confirm(
        `Repoint this workspace to revision ${safeSlice(row.revision_id, 8)}? This is a manual ` +
