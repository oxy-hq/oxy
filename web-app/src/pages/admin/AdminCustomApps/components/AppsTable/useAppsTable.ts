...
const SOURCE_LABEL: Record<CustomApp["source_type"], string> = {
  v0: "v0",
  local: "Local",
  s3: "S3"
};
 ...
 * the grouping.
 */
export function buildAppsTableModel(apps: CustomApp[], state: AppsTableState): AppsTableModel {
...
export interface FleetStats {
  ...
  draft: number;
  orgs: number;
  bySource: Record<CustomApp["source_type"], number>;
}
 ...
 */
export function fleetStats(apps: CustomApp[]): FleetStats {
...
function compareBy(key: SortKey, a: CustomApp, b: CustomApp): number {
  switch (key) {
    case "name":
...
function partition(sorted: CustomApp[], group: Exclude<GroupBy, "none">): AppGroup[] {
...
function groupKey(a: CustomApp, group: Exclude<GroupBy, "none">): string {
...
function groupLabel(key: string, group: Exclude<GroupBy, "none">): string {
  ...
  return SOURCE_LABEL[key as CustomApp["source_type"]] ?? key.toUpperCase();
}

const statusRank = (a: CustomApp): number => (a.published_at ? 1 : 0);
