/**
 * Right-side output panel for the workflow run page.
 *
 * Mirrors the legacy `WorkflowOutput` layout: PanelHeader with an X close
...
import { useLogItems } from "@/hooks/api/agentic-workflows/useLogItems";
import type { WorkflowEvent } from "@/services/api/agenticWorkflows";
...
type Props = {
  ...
  /**
   * Full per-step state from `useWorkflowRunStream`. Used by the
   ...
   * button is hidden — only the workflow page's mobile layout wires
   * this up today.
   */
...
export const RunSidebar: React.FC<Props> = ({
  ...
  //
  // Icon convention matches the legacy `WorkflowOutput`:
  ...
  return (
    <Panel>
      ...
      <div className='flex shrink-0 items-center justify-between gap-3 border-b px-4 py-2'>
        <RunSelector workflowRef={workflowRef} runId={runId} onSelect={onSelectRun} />
        <div className='flex items-center gap-2'>
          ...
          />
          <Label htmlFor='show_workflow_logs' className='text-xs'>
            Show logs
          </Label>
        </div>
      </div>
...
function phaseLabel(phase: WorkflowRunStream["phase"]): string {
  switch (phase) {
    case "idle":
      return "Idle";
    case "starting":
