...
- Headless mode by default (use `HEADED=1` to override)
- Parallel test execution: 50% CPU cores locally, 2 workers on CI
- Optimized timeouts: 30s navigation, 10s actions, 5s assertions
- Video recording disabled (screenshots on failure only)
...
await idePage.insertTextAtEnd("New content");
await idePage.saveFile();
```

**Note:** Use the dedicated `test-file-for-e2e.txt` for IDE editing tests to avoid git conflicts.
