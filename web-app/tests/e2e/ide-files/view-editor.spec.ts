...
test.describe("IDE Files - View Editor - Explorer Mode Fields Panel", () => {
  ...
  test("9.1 - should display dimensions and measures when view loads", async ({
    page,
  }) => {
  ...
  test("9.5 - should highlight and add dimension to query when clicked", async ({
    page,
  }) => {
    await page.getByRole("tab", { name: "Objects" }).click();
    await page.waitForTimeout(500);
  ...
  test("9.9 - should handle view with many fields via virtual scroll", async ({
    page,
  }) => {
    await page.getByRole("tab", { name: "Objects" }).click();
    await page.waitForTimeout(500);
    ...
    if (await semanticSection.isVisible()) {
      ...
      if (await viewFile.isVisible()) {
        ...
        const fieldsPanel = page
          .locator('[data-testid*="fields"], .fields-panel')
          .first();
...
test.describe("IDE Files - View Editor - SQL & Execution", () => {
  ...
  test("9.22 - should generate SQL when fields are selected", async ({
    page,
  }) => {
    const semanticSection = page.getByText("Semantic Layer");
    if (await semanticSection.isVisible()) {
      ...
      if (await viewFile.isVisible()) {
        ...
        if (await field.isVisible()) {
          ...
          const sqlPanel = page
            .locator('[data-testid*="sql"], .sql-panel, .monaco-editor')
            .first();
  ...
  // 9.25 Execute query
  test("9.25 - should display results when query is executed", async ({
    page,
  }) => {
    const semanticSection = page.getByText("Semantic Layer");
    if (await semanticSection.isVisible()) {
  ...
  test("9.27 - should show no results message for empty query", async ({
    page,
  }) => {
    const semanticSection = page.getByText("Semantic Layer");
    if (await semanticSection.isVisible()) {
...
test.describe("IDE Files - View Editor - Loading Errors", () => {
  ...
  test("9.33 - should show loading state while fetching view data", async ({
    page,
  }) => {
    const semanticSection = page.getByText("Semantic Layer");
    if (await semanticSection.isVisible()) {
