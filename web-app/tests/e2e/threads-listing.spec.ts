...
import { resetProject, seedThreadsData } from "./utils";

test.describe("Threads Listing Page", () => {
  test.beforeEach(async ({ page }) => {
    resetProject();
    // Seed 15 threads to ensure pagination and tests work reliably
    await seedThreadsData(page, 15);
  ...
  test("should navigate to next page when pagination clicked", async ({
    ...
    if (await page2Link.isVisible()) {
      ...
      // Note: This test depends on having enough threads to paginate
      await page.waitForTimeout(1000); // Wait for new threads to load
      await expect(
        page.locator('[data-testid="thread-item"]').first(),
      ).toBeVisible();
    }
