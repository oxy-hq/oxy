...
- **Custom Apps Platform** (code-first React+Vite bundles, shipped with `oxy publish`) is **not** YAML **Data Apps** (`.app.yml` dashboards). User-facing copy says **"custom app"**, but routes and hosts still say `customer-apps`. Local dev against live cloud data goes through an `oxy proxy` sidecar (side-effecting function/agent calls held, analytics dropped). Usage analytics are on by default (opt out per build), and the `oxy-` event-name prefix is reserved for platform metrics — an app emitting one gets `400`s.
...
- **`ctx.oltp`** = a **third data plane** — per-org transactional Postgres, not the warehouse and not the semantic layer. Each writer (a custom app, or an Airway pipeline landing raw extracts) owns one schema and can't see another's; defaults differ on purpose — a pipeline's `raw_*` schema is analytics-agent-readable, an app's `app_*` schema is private. A slug becomes a schema/role name, so underscores are rejected and an app can't be renamed or deleted while its writer is provisioned.
...
- **Admin → Workspace Health** — **opt-in per workspace**: no `health_check:` block in `config.yml` — or an unparseable one — means the workspace is never evaluated and is *absent* from the admin table rather than shown healthy. Only an unhealthy *transition* pages Slack, never "degraded".
...
- **World Model Graph** (Globe icon in the rail) — map of the semantic layer, driven by `.world-model.yml`. Distinct from the **Context Graph** and the **Metric Tree** — three different graph surfaces. Selecting a measure also sizes **opportunities**; its refusals are deliberate — per-unit rates only (`gap × volume`, never raw totals), significant segments only, no denominator means refused rather than sized — so an empty list is usually right.

---

## Components worth clarifying

- **Agentic Agent** — when the semantic layer lacks a measure it needs, it answers from what exists rather than handing off to the Builder Agent mid-run. It is **freshness-aware** — prompts carry per-source coverage from a view's `meta:` contract, so an answer scoped to "data covers through *date*" is honest, not an under-report.
