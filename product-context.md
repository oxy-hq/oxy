...
transformation: connect a warehouse, ask questions in chat where agents generate
and run SQL, and get streamed results. Teams also build **Automations** (YAML),
...
- **Cloud / enterprise** (`oxy serve`, `oxy serve --enterprise`, `oxy start` for a Docker-Postgres dev box) — the real, maintained path: multi-tenant, multi-org, RBAC, magic-link auth, Stripe billing (`ServeMode::Cloud`). **"Run it locally" means this.** So dev-vs-prod is *not* distinguishable by mode — a dev box is cloud mode with non-prod secrets (no working SES → `OXY_APP_EMAIL_LOCAL_TEST=1` previews email).
...
- **Custom Apps Platform** (code-first React+Vite bundles, shipped with `oxy publish`) is **not** YAML **Data Apps** (`.app.yml` dashboards). User-facing copy says **"custom app"**, but routes and the `<org>--<slug>.customer-apps.oxygen-hq.com` host still say **"customer app"** / `customer-apps`. Local dev against live cloud data goes through an `oxy proxy` sidecar (like `vercel dev`): analytics events are dropped, side-effecting function/agent calls held unless confirmed.
- **Oxy Functions** = server-side TypeScript handlers declared in `oxy-app.json` and shipped *inside* a Custom App bundle (frontend and backend promote and roll back together) — **not** a YAML Automation task or Data App `task`. One runs as an HTTP route (`useFunction`), a cron job on the durable task queue, or an Airway transform step. Side-effecting powers are **capability-gated in the manifest and fail closed**: `secrets.write` for `ctx.secrets.set`, `email.send` for `ctx.email.send` (SES; the **platform** owns `from`, the function sets only `replyTo`), `storage.read/write` for `ctx.storage` (private per-app asset silo, presigned URLs only, no public access). Email templates are **preact** rendered by `@oxy-hq/sdk/email` — React Email / react-dom can't run in the Functions isolate (node:stream / Web Streams).
...
- **Two subdomain schemes, easy to confuse** — an **org subdomain** (`<org-slug>.oxygen-hq.com`) boots the whole product pre-scoped to that org's default project (no org/workspace picker), serving its custom apps under `/a/<slug>/`; a **custom-app subdomain** (`<org>--<slug>.customer-apps.oxygen-hq.com`) serves one externally-hosted app at its own root.
...
- **Home / HQ launcher** (`/`, `/home`) — apps-first landing (org-branded **HQ**, a grid of **custom-app cards**). **Ask Oxygen** (⌘K) opens a resizable right-side **drawer** that compacts the page beside it (not a floating overlay); "Full view" promotes to `/threads/:id`. Chrome is a top bar plus an **icon rail** (Home · **Chat** · Apps), no left sidebar; the rail's **Chat** entry was formerly "Threads".
- **Thread** (`/threads/:id`) — messages carry free-text plus structured artifacts (`execute_sql` shows the SQL the agent ran).
- **Apps** (`/apps/:id`) — a Data App; Controls inject Jinja values and re-run dependent tasks; results cached by parameter hash (`?refresh` forces a re-run).
...
- **Admin → Workspace Health** — five passive dimensions plus **smoke tests** that really run a connection / topic / app / agent probe, so the data- and token-spending ones are opt-in. Only an unhealthy *transition* pages Slack, never "degraded".
- **World Model Graph** (Globe icon in the rail) — interactive map of the semantic layer (entities, measures, promotion up the hierarchy), driven by `.world-model.yml`. Distinct from the **Context Graph** and the **Metric Tree** — three different graph surfaces. Selecting a measure also sizes **opportunities**, whose refusals are deliberate: per-unit rates only (`gap × volume`, never raw totals), only segments passing a significance test, and a measure with no per-unit denominator is refused rather than sized — an empty list is usually correct by construction.

---

## Components worth clarifying

- **Agentic Agent** — multi-step FSM pipeline, not one LLM call; supports **human-in-the-loop suspension** (pauses on a clarifying question, then resumes). When the semantic layer lacks a measure it needs, it answers from what exists rather than handing off to the Builder Agent mid-run.
- **Builder Agent** — sends **targeted line edits**, not full-file rewrites; multi-file edits confirm/reject **per file**. Also powers first-run **Workspace Onboarding** (generates views/topics/apps, smoke-tests each end-to-end, self-corrects, survives a restart).
- **Semantic Layer** — `.view.yml` / `.topic.yml` compiled by **airlayer** with automatic join resolution and fan-out protection. Queries may serve from local **pre-aggregation** Parquet rollups instead of the warehouse; a **Pre-aggregated** badge means it came from a rollup (stale data under that badge is a freshness bug).
...
- **Metric Tree & Anomaly Monitoring** — a `.monitor.yml` watches a measure over time (per-segment via `filters`/`group_by`); anomalies land in the **Insights Inbox** with AI root-cause, also reachable from chat and the SDK. The **Metric Tree** (a Semantic Layer IDE tab) decomposes a top-line metric into driver metrics. Two deliberate silences that read as bugs: scans exclude the current *incomplete* period, and a series isn't scanned at all until it has **~8 seasonal cycles of history** (≈8 weeks daily), so a new segment stays quiet rather than reporting its own opening ramp. **Explain** compares the same phase one cycle back (Monday vs prior Monday), scopes to the segment that fired, and prunes time-part / row-key dims so day-of-week or `*_id` is never a "driver."
...
- **Empty-result warehouse queries** can panic in the shared Arrow bridge (DuckDB / Snowflake / MotherDuck / connectorx); each path must short-circuit its empty shape. Oversized results hit a cross-connector memory backstop and unbounded semantic/SQL-IDE queries cap at 10k rows — both flag **truncated**, so "missing rows" may be a silent cap, not a query bug.
...
- **Two distinct worker concepts, easy to conflate** — the *durable task fleet* (in-process by default, standalone via `oxy worker`) executes queued `TaskSpec` jobs; the *global singleton worker* (`OXY_INPROC_GLOBAL_WORKER`) drives schedules, monitor scans, and pre-aggregation. Toggling one does not affect the other, and with the singleton off **schedule CRUD works but nothing ever fires**. Multi-replica safety is a CAS `next_run_at` claim (no double-fire; missed runs collapse to one); cross-process cancel is a polled flag.
...
- **Serve ports are `OXY_HTTP_PORT` / `OXY_HTTP_INTERNAL_PORT`** — renamed off `OXY_PORT` / `OXY_INTERNAL_PORT`, which collided with Kubernetes-injected service-discovery vars and could stop a self-hosted deploy booting.
...
- **Input that must be sanitized/allowlisted** — DuckDB config SQL escaped against quote injection; the Slack re-post handler allowlists static block kinds and drops interactive ones; the `http_request` automation task is HTTPS-only and blocks localhost / cloud-metadata / private-IP egress unless a per-task `allow_hosts` opts in. Uploaded custom-app bundles are size-capped **on unpack** — one serve instance hosts many apps.
...
`.agentic.yml` · `.automation.yml` (+ legacy `.procedure.yml`) · `.app.yml` · `.view.yml` / `.topic.yml` · `.sql` · `.airway.yml` · `.world-model.yml` · `oxy-app.json` (custom-app manifest: functions, schedules, capabilities, icon/art) · `oxy.yml` under `modeling/<project>/` (dbt target → Oxy connection map). Two with semantics you can't guess:
...
- **`reconcile.yml`** — root-only singleton; compares an Oxy measure (semantic **or** raw scalar `sql:`) against a live external source (Toast) with abs+pct tolerance, driving the admin workspace-health **Reconciliation** dimension. Same `timezone` / `freshness` semantics, same `date`-column inertness.
