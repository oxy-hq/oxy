...
connect a warehouse and ask questions in chat, where agents write and run SQL. Teams
also build **Automations** (YAML), **Data Apps** (YAML dashboards), **custom apps**
(code-first React bundles), and dbt-style transforms via **Airform**.
...
- **Custom Apps Platform** (code-first React+Vite bundles, shipped with `oxy publish`) is **not** YAML **Data Apps** (`.app.yml` dashboards). User-facing copy says **"custom app"**, but routes and hosts still say `customer-apps`. Local dev against live cloud data goes through an `oxy proxy` sidecar (side-effecting function/agent calls held, analytics dropped).
- **Oxy Functions** = server-side TypeScript handlers declared in `oxy-app.json` and shipped *inside* a Custom App bundle (frontend and backend promote and roll back together) — **not** a YAML Automation task or Data App `task`. Side-effecting powers are **capability-gated in the manifest and fail closed**: `secrets.write`, `email.send` (the platform owns `from`; the function sets only `replyTo`), `storage.read/write` (private per-app silo, presigned URLs only). Isolate gotchas: email templates must be **preact** via `@oxy-hq/sdk/email` (react-dom needs node:stream); no `Buffer`/`TextEncoder` (use `btoa`/`atob` or the SDK's helpers); `ctx.fetch` decodes as UTF-8, so a binary body needs `encoding: "base64"` or it silently corrupts; an app's **first** `storage.retention` rule changes how uploads are signed, so a non-SDK uploader starts failing opaquely; and `ctx.tx()` is **Postgres-only** — every other warehouse refuses a transaction by name rather than emulating one, and a `numeric` column must be cast (`amount::text`) rather than read back as a float.
...
- **Global Owner** (`OXY_OWNER` env allow-list → `is_owner`) is root — everything, incl. the Billing queue and Global-admin management. **Global Admin** (role `global_admin`, seeded by `OXY_GLOBAL_ADMINS`) is Oxy ops — most of admin + every custom app, but **not** those two.
- **App Operator** (role `app_operator`) — ships and develops custom apps and **nothing else**, optionally scoped to named orgs. An out-of-scope request answers **not found** rather than *not allowed* so scope can't be probed — a 404 in an admin surface may be a scope boundary, not a missing row. A grant you issue must be strictly weaker than your own.
...
- **Home / HQ launcher** (`/`, `/home`) — apps-first landing listing only the **custom apps the viewer can actually open**. **Ask Oxygen** (⌘K) is a right-side drawer; "Full view" promotes to `/threads/:id`. The rail's **Chat** entry was formerly "Threads".
...
- **Admin → Workspace Health** — **opt-in per workspace**: no `health_check:` block in `config.yml` — or an unparseable one — means the workspace is never evaluated and is *absent* from the admin table rather than shown healthy. Passive dimensions plus opt-in **smoke tests** that really run probes. Only an unhealthy *transition* pages Slack, never "degraded".
- **World Model Graph** (Globe icon in the rail) — map of the semantic layer, driven by `.world-model.yml`. Distinct from the **Context Graph** and the **Metric Tree** — three different graph surfaces. Selecting a measure also sizes **opportunities**, and its refusals are deliberate: per-unit rates only (`gap × volume`, never raw totals), significant segments only, and no per-unit denominator means refused rather than sized — so an empty list is usually right.

---

## Components worth clarifying

- **Agentic Agent** — when the semantic layer lacks a measure it needs, it answers from what exists rather than handing off to the Builder Agent mid-run; it can also suspend on a clarifying question and resume. It is **freshness-aware** — prompts carry per-source coverage from a view's `meta:` contract, so an answer scoped to "data covers through *date*" is honest, not an under-report.
...
- **Metric Tree & Anomaly Monitoring** — a `.monitor.yml` watches a measure over time (per-segment via `filters`/`group_by`); anomalies land in the **Insights Inbox** with AI root-cause. The separate **Metric Tree** (a Semantic Layer IDE tab) decomposes a top-line metric into drivers. Two deliberate silences that read as bugs: scans exclude the current *incomplete* period, and a series isn't scanned until it has **~8 seasonal cycles of history** (≈8 weeks daily), so a new segment stays quiet rather than reporting its ramp. Severity is *distance past the envelope edge* — the same quantity that decided the firing, so ordering and filtering agree; too few same-phase points means no opinion, floored at **Medium**. Many segments breaching the same bucket group into one **cohort**; a `calendar:` block only *labels* a cohort, never filters. A wholly-dismissed event deliberately offers no **Ack** in the **All** tab — un-dismissing happens from **Dismissed**, so an Ack can't reverse a dismissal you can't see from there. **Explain** compares the same phase one cycle back (Monday vs prior Monday), scopes to the segment that fired, prunes time-part dims, and keeps a `*_id` driver when the semantic layer declares it an **entity** (the name heuristic is only a fallback). Drivers sort into explaining / offsetting / **mechanical** / undetermined — one that merely tracks its base is mechanical, neither cause nor offset.
...
- **Design system** — semantic tokens, never raw hex; **emerald is reserved for workflow-node success only**.
...
- **Observability is ClickHouse-only** — no default in any mode (`--local` included), so unset means capture is simply off; a removed `duckdb` / `postgres` / `airhouse` label errors at boot and starts with observability *disabled*, reading as "no traces" rather than crashing.
- **Observability serving has two repeat footguns** — timestamps must go out as ISO-8601 UTC or the browser mis-parses them (render crash, spans collapsed to slivers), and trace queries need hard time/size caps: an unbounded scan took the backend offline, not merely timed out. Captured spans surface ~30s later by design.
...
- **Input that must be sanitized/allowlisted** — the `http_request` automation task is HTTPS-only and blocks localhost / cloud-metadata / private-IP egress unless a per-task `allow_hosts` opts in; DuckDB config SQL is escaped against quote injection. Uploaded custom-app bundles are size-capped **on unpack** — one serve instance hosts many apps.
...
- **`reconcile.yml`** — root-only singleton comparing an Oxy measure against a live external source (Toast), feeding the workspace-health **Reconciliation** dimension. Same `timezone` / `freshness` semantics as `.monitor.yml` — but on a `week`/`month` grain a `freshness` under one full grain buys a settle time that swings with the weekday, so the check reconciles for days and then reports drift that isn't there.
