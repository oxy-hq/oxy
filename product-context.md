...
- **Orchestrator Dashboard** — A three-tab unified surface for managing scheduled and ad-hoc runs across workflows, ELT pipelines, and agents (replaces the previous Coordinator surface).
  - **Overview tab** — Health cards, a swimlane timeline (lanes per run type, "now" marker) with a board toggle, and an anomaly feed for failures and duration spikes.
  - **Jobs tab** — Catalog of every schedule with filter/search, enable toggle, run-now (with confirm), edit, and delete. A **Job Detail** view shows the schedule definition with the next five runs in its timezone, a health card with missed-run audit, a **Backfill** button (with live blast-radius computation), and per-job run history.
  - **Runs tab** — URL-synced filters (copy the URL → filters survive), a trigger column labelling runs as `scheduled` / `manual` / `backfill` / `retry`, list-level cost hints on agent rows with LLM activity, bulk retry and cancel, and a `System: off/on` toggle that hides preagg cycles and other system-managed runs by default.
  - **Polymorphic Run Detail** — Agent runs show an LLM Usage card (tokens in/out/cache, dollar cost, models); workflow runs show a per-step timing card with hot-spot marker; ELT runs show a per-table row-count card with extract/load phase durations and drop warnings.
  - Every coordinator endpoint is scoped by `workspace_id` so multi-tenant deployments don't expose runs across workspaces.
...
- **Workspace Management** (multi-workspace mode) — Import repositories from GitHub, switch workspaces, invite members with role-based access (Owner/Admin/Member). Workspace effective role is derived from the user's org role via `EffectiveWorkspaceRole`.
- **Roles & permissions** — Oxy distinguishes **platform-level** roles ("Global …") from **per-org** roles. The same word ("admin", "owner") names different things at the two levels; the UI uses the **Global** prefix for the platform-level versions so the distinction is visible to operators.
  - **Global Owner** — Oxy staff with the highest privileges. Backed by the `OXY_OWNER` env var (comma-separated email allow-list), surfaced as `is_owner: true` on `/api/users/me`. Reaches **every** admin page including Billing queue and Global admins (the page that adds/removes Global Admins).
  - **Global Admin** — Oxy operations role. Backed by the `app_admins` DB table (seeded from `OXY_GLOBAL_ADMINS` env var on first boot; the legacy `OXY_APP_ADMINS` name is still accepted with a deprecation warning). Surfaced as `is_app_admin: true`. Reaches most of the admin panel (Feature flags, Internal jobs, Customer apps, Organizations, Users, Workspaces) and every registered customer app regardless of org membership. **Cannot** reach Billing queue or manage other Global Admins — those stay reserved for Global Owners.
  - **Org Owner** — highest tenant-internal role inside a single organization. Stored as `role = "owner"` in `org_members`. Manages the org's members, billing, workspaces. Has **no platform-level reach** by virtue of being Owner; if they also need `/admin/*` access they must be added as a Global Admin or Global Owner.
  - **Org Admin** — second tenant-internal role. `role = "admin"` in `org_members`. Manages the org's members and workspaces but not billing.
  - **Org Member** — base tenant role. `role = "member"` in `org_members`. Read/write within workspaces they're added to.
  - **Backend identifier crosswalk** (the on-the-wire names didn't change with the UI rename):
    - `OXY_OWNER` env var → user info `is_owner` → UI label **Global Owner**.
    - `OXY_GLOBAL_ADMINS` env var (or `OXY_APP_ADMINS` legacy) → seeds `app_admins` table → user info `is_app_admin` → UI label **Global Admin**.
    - Per-org role → workspace-effective role via `EffectiveWorkspaceRole`.
  - **Middleware split** (`crates/app/src/server/api/middlewares/`): `oxy_owner_guard` (strict Global Owner only — Billing + Global admins routes), `oxy_app_admin_guard` (strict Global Admin only — used by the parallel `/customer-apps` surface), `oxy_owner_or_app_admin_guard` (permissive Owner OR Admin — outer layer on `/admin/*`).
...
- **Cron Scheduling** — Workflows, Airway ELT pipelines, and analytics agents can run on a cron schedule managed from the Schedules settings panel. Agent schedules carry a required `question` field so each tick fires the agent with a fixed prompt. Schedules tick through a CAS-based `next_run_at` claim, so multiple Oxy processes can run side by side without double-firing; missed runs from extended downtime collapse to a single execution rather than replaying. Cross-process cancel uses a persistent `cancel_requested_at` flag polled by the executing worker, so cancels are no longer tied to the originating HTTP connection. Per-schedule observability surfaces last error (with destructive icon/tooltip when set, cleared on success) and a duration-spike anomaly feed. Schedules and runs are workspace-scoped. Scheduled firing is gated behind the `OXY_INPROC_GLOBAL_WORKER` environment flag — with the flag off, schedule CRUD still works but the periodic loop, latency worker, and tick do nothing.
- **Semantic Pre-Aggregation Rollups** — Semantic-layer queries can be served from locally-materialized Parquet rollups instead of round-tripping to the warehouse. A coordinator-driven background worker evaluates each rollup's `refresh_key`, rebuilds stale rollups in parallel, and materializes results to local Parquet via hot-swap rename. The local Parquet fast path is wired into every place semantic queries execute: workflow and procedure `semantic_query` steps, the IDE **Run** button on `/semantic`, the analytics agent pipeline (serial and fan-out execution), and the Builder's `semantic_compiler` tool — falling back to the warehouse on misses. Three-layer freshness is verified via in-memory cache, the on-disk manifest's `build_date` (which survives restarts), and a SQL `refresh_key` check against the warehouse. `QueryExecuted` events carry an `is_preagg` flag so the UI surfaces a **Pre-aggregated** badge on analytics SQL blocks, workflow `semantic_query` results, and procedure runs. The semantic explorer surfaces rollup detail with click-to-apply dimensions and measures. Operator-configurable `pre_aggregations.refresh_worker.renewal_threshold` controls how aggressively the worker rebuilds.
- **Git Subdirectory Workspaces** — Workspaces can live in a subdirectory of a git repo, not just at the repo root. Workspace tooling walks up from the workspace directory to find the actual `.git` entry (mirroring git's own discovery behavior), and IDE GitHub links include the subdirectory path (`/tree/<branch>/packages/my-app`).
...
- **Cron schedule double-firing / replays** — Multi-replica deployments rely on the CAS-based `next_run_at` claim to avoid double-firing; missed runs after extended downtime must collapse to a single execution, not replay every missed tick. Regressions surface as duplicate runs in the Runs tab or a burst of catch-up firings. Scheduled firing is gated behind `OXY_INPROC_GLOBAL_WORKER` — flipping this flag accidentally either disables scheduling entirely or wakes the worker in environments that shouldn't run it.
- **Cross-process scheduled run cancel** — Cancelling a scheduled run from the dashboard must work even when the run is executing on a different process. Cancels rely on the persistent `cancel_requested_at` flag being polled by the executing worker; if a long-running step ignores or fails to check the flag, cancel silently stalls.
- **Pre-aggregation freshness and crash recovery** — Rollup freshness is verified through three layers (in-memory cache → on-disk manifest `build_date` → warehouse `refresh_key`). Skipping any layer surfaces as stale data shown with a **Pre-aggregated** badge. Pre-aggregation runs left in `running` at startup must be marked failed so the next tick can retry from scratch, instead of blocking the worker indefinitely. Empty rollups must not poison the manifest. Extremely large `Every` interval values must not cause spurious continuous rebuilds — the overflow case is treated as "always fresh" with a logged warning.
- **Pre-aggregation workspace scoping** — Pre-aggregation runs carry a `workspace_id` so the worker schedules and tracks them per tenant. Code paths that insert pre-aggregation runs without `workspace_id` will compile-fail today, but regressions could leak runs across tenants in multi-workspace deployments.
- **Orchestrator endpoint workspace scoping** — Every Orchestrator endpoint (list, active, tree, retry, recovery, queue, live SSE) must be scoped by `workspace_id` from the URL. A missing filter exposes runs across workspace boundaries on multi-tenant deployments.
- **Git subdirectory workspace operations** — Workspaces inside a subdirectory of a git repo must walk up to find the actual `.git` entry, mirroring git's own discovery behavior. IDE GitHub links must include the subdirectory path (`/tree/<branch>/packages/my-app`); regressions surface as "404 not found" GitHub links or git operations resolving to the wrong repo when a workspace isn't at the repo root.
