...
| `.monitor.yml` | Anomaly monitor | Watches a measure over time; per-granularity `schedule:`; gated by `OXY_INPROC_GLOBAL_WORKER`. `timezone` only bites on a `type: datetime` time dimension — a `type: date` business-date column is already a local calendar date and is bucketed raw |
| `reconcile.yml` | Reconciliation checks | Root-only singleton; compares Oxy measures to a live external source (Toast) with abs+pct tolerance; drives the admin workspace-health **Reconciliation** dimension. Windows take a wall-clock `freshness` offset (`3d`) for late-arriving data and a `timezone` for non-UTC business calendars — `timezone` only bites on a `type: datetime` time dimension; on a `type: date` one airlayer passes the column through raw, so it is inert |
