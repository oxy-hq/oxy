...
like to add avocado?", "want to make that a large?") — as opposed to
ordinary order-taking, confirming an order, or merely mentioning an item.

That's the feature's core novel risk: an item word alone ("avocado") is not
an upsell; the *offer* is. We use the same Haiku model the edge already
calls for the VLM, with a forced structured tool call so the output is
always a typed verdict. No audio or transcript is persisted — text in,
verdict out. Runtime port of `video-poc/audio-poc/intent.py`.
"""
...
# Same model family the edge uses for the VLM (worker/main.py VLM_MODEL).
...
def classify(transcript: str, *, client: anthropic.Anthropic | None = None) -> Verdict:
    ...
    returns a non-upsell verdict flagged with `.error` so the audio reader
    keeps running (a missed detection beats a crashed stream)."""
